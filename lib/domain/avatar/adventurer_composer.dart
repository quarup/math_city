...
String composeAdventurer(AdventurerConfig config) {
  ...
  final earringsVariant =
      kEarringsOptions[config.earringsIndex.clamp(
        0,
        kEarringsOptions.length - 1,
      )];

  final base = kAdventurerBase.replaceAll('__SKIN__', skinHex);
  final hair = (kAdventurerHair[hairStyle] ?? '').replaceAll(
    '__HAIR__',
    hairHex,
  );
  ...
  final glassesSvg = glassesVariant != null
      ? (kAdventurerGlasses[glassesVariant] ?? '')
      : '';
  final earringsSvg =
  ...
  final frecklesSvg = config.freckles
      ? (kAdventurerFeatures['freckles'] ?? '')
      : '';
