...
- **Next action:** **Start Phase 10 on a branch, domain first.** Order: (1) `ConstructionSite` domain type + rules with tests (pay-in, stage fraction, cap, delta pricing, `upgradesFrom`); (2) schema bump dropping `coinBalance`; (3) reward routing into the active site; (4) city-screen sites + HUD bar; (5) camera zoom + wheel overlay + summary card; (6) beat → *Build it* highlight and the first-run path; (7) upgrades; (8) generic stage art. Resolve the §8.11 open questions as they're hit (cancel semantics first — it shapes the schema). Then land as a PR and `/ux-sweep` the new site endpoint.
...
- [ ] **Domain (pure Dart, tested first):** `ConstructionSite` value type + rules — pay-in from an `AnswerReward`, paid fraction → stage index, open-on-full, the 3-open-sites cap, upgrade delta pricing (`target.coinCost − source.coinCost`), `upgradesFrom` link, net-population on open. Land blocks and parks are sites too.
- [ ] **Schema bump (wipe-and-recreate, pre-launch precedent):** `ConstructionSites` table (cityId, buildingTypeId, gridX/Y, paidCoins, upgradesFromPlacementId?, startedAtRound); **drop `Players.coinBalance`** (`lifetimeCoinsEarned` and `streakCount` stay); retire the in-place tier columns.
