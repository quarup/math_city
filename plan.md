...
- **Next action:** **Phase-9 art-pipeline — continue the rollout from the validated pilot.** (1) **Cull variants** to the §5.4 target counts: review the `tools/sprite_pipeline/debug/` overlays for the processed buildings (`single_home` has 6 raws, §5.4 target 4–5; `apartment` 5; `coffee_shop` 4), drop the weakest, delete the matching `assets/buildings/<id>_v<n>.png`, and lower `numVariants` in [building_registry.dart](lib/domain/city/building_registry.dart) to the kept count. (2) **Decide the binary-asset commit strategy** — `tools/sprite_pipeline/raw/` (37 jpegs ≈ 21 MB; README calls it source-of-truth) + the processed PNGs are still uncommitted. (3) **Process + wire the remaining 10 raw building types** (`amusement_park`, `aquarium`, `city_hall`, `duplex`, `high_rise`, `library`, `observation_tower`, `post_office`, `town_hall`, `townhouse_row`) — these are Phase-9-only and need new [building_registry.dart](lib/domain/city/building_registry.dart) entries (costs / unlock rules / footprints + demand beats per city_builder.md §3/§4) before they can render. (4) Generate + process the rest of the 55-building / 97-variant catalog. **Gotcha hit this session:** iCloud/Finder ` 2`-suffixed duplicate-file pollution under `build/` + `.dart_tool/` broke the first Android build (`packJniLibs` zip conflict on `libsqlite3 2.so`); `flutter clean` + `find . -name '* 2.*' -delete` fixed it, recurs if the Documents folder re-syncs mid-build. **Phase-7 device-verification (non-sprite) still partly owed** — pan/zoom feel, tap-to-place accuracy, move-mode pick-up/drop highlight, citizen-bubble rotation/ack, demand-gated reveal, age-gated milestone bubble; most are now incidentally observable on the emulator. Once swept, run the **Phase-7 exit-criteria playthrough**. Phase 6 deferred work (selective DeepMind expansion, mix-policy + adaptive-system tuning, kid validation) revisited post-Phase-7. *(Variant-cull to §5.4 target counts deferred — all 16 pilot variants kept for now.)*
