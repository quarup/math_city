...
- **Phase:** Phase 6 — Full Question Bank. **169 generators implemented**; broad K–8 coverage. Remaining gaps: coordinate plane, transformations, data displays (bar/box/histogram), compound probability, some 3D volume (cylinder/cone/sphere need π), most K–2 counting/place-value foundations.
- **Last updated:** 2026-05-17
- **Last action:** **Chunk 23 — compare integers + powers of 10** — 5 generators in `place_value_generators.dart` plus two override cleanups. Compare: `compare_2digit` (G1), `compare_3digit` (G2), `compare_multidigit` (G4) — all share `_compareIntsGen("Which is bigger: a or b?")` MC shape with `wrong`, `"They are equal"`, and a misconception-difference distractor that falls back to the sum when `wrong = |a − b|` (e.g. for 10 vs 5). Place value: `place_value_relationship_10x` (G5, "In 770, how many times greater is the value of the leftmost 7 than the rightmost 7?" — builds a number with the repeated digit at gap `k ∈ [1, 3]` and zeros elsewhere; answer = 10^k; misconception distractor = `k × 10`), `powers_of_10` (G5, three shapes: `What is 10^k?`, `a × 10^k = ?`, `N ÷ 10^k = ?`). Override cleanup: removed the `mult_facts_within_100` substitutes for both `exponents_whole_number` and `scientific_notation_read` — their canonical `powers_of_10` prereq now lives. New override: `compare_multidigit` drops the unimplemented `read_write_multidigit`, substitutes `place_value_multidigit`. Tests: +5 groups @ 300; **349 total pass**, `flutter analyze` clean.
