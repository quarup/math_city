...
- **Phase:** Phase 6 — Full Question Bank. **95 generators implemented**; four K–8 categories with broad coverage (`fractions` 26, `decimals_percent` 24, `ratios` 10 of 16, `expressions_equations` 5 of 14). Word problems + rationals also covered. Geometry and statistics still untouched.
- **Last updated:** 2026-05-17
- **Last action:** **#12 Equations / order-of-ops (chunk 8)** — new file `equation_generators.dart` with 5 generators: `order_of_operations_no_exp` (G5, three-operand "a op1 b op2 c"; LTR for equal-precedence ops — bug fixed: `pre2 > pre1`, not `>=`), `nested_grouping` (G5, parens-on-left vs parens-on-right variants; re-rolls if parens don't actually change the result), `evaluate_expression` (G6, "ax + b, x=?"; prereq override drops `order_of_operations_with_exp`), `solve_one_step_eq_addition` (G6, `x ± p = q`; override drops `missing_addend_within_20` + `write_expression_from_words`; substitute `add_within_100`), `solve_one_step_eq_mult` (G6, `px = q` or `x ÷ p = q`; override substitutes `mult_facts_within_100`). Tests: +5 groups @ 300; **274 total pass**, `flutter analyze` clean.
- **Last action (prior):** **#8 Ratios (chunk 7) — cleanup** — `unit_pricing`, `convert_units_using_ratio`, `proportional_relationship`, `constant_of_proportionality`, `proportional_equation`. Commit `0e90c14`.
...
- **Next action:** Pick the next sub-slice. Easy options remaining ([curriculum.md §5.1](curriculum.md)):
  - **#12 Equations chunk 9** — `solve_two_step_eq` (G7), `expand_linear_expression` (G7, "Expand 3(x+4)"), `add_subtract_linear_expressions` (G7, "Simplify 3x + 2 + 5x − 1"), `equivalent_expressions_props` (G6), `substitute_to_check` (G6). All pure algorithmic.
