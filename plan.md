...
- **Phase:** Phase 6 — Full Question Bank. Fractions slice complete: 19 sub-concepts (the original 4 rewritten + 15 new) covering equivalence/comparison, simplification, mixed-number conversion, +/− with like and unlike denominators, multiplication (with new `area_grid` diagram), division (unit-fraction and fraction-by-fraction), and fraction-as-division word problems. Issue [#2](https://github.com/quarup/math_city/issues/2) (equivalent fractions accepted) resolved. **45 generators implemented**; one K–8 category fully covered (`fractions`), with decimals/percentages, ratios/proportions, geometry, prealgebra, and statistics still untouched.
- **Last updated:** 2026-05-12
- **Last action:** Fraction sub-slice (#3 in the priority list). **Foundation** (commit `96e4ea9`): new `Fraction` value type in [lib/domain/questions/fraction.dart](lib/domain/questions/fraction.dart) with parse / equalsByValue / reduce / toCanonical / toMixed. New `AnswerFormat` enum (integer | fraction | mixedNumber | string) on `GeneratedQuestion`. `checkAnswer` returns canonical | equivalentNonCanonical | wrong; question screen routes through it; result screen renders a friendly `_EquivalentNudgeCard` ("You said 2/4 — that's equal to 1/2!") on non-canonical equivalent answers. Keypad max-length bumped 7 → 8; space char inserted as the mixed-number separator. Existing 4 generators rewritten: `add_fractions_like_denom` now emits canonical (reduced) sums. **Batch 1** (`ae6b36e`): `equivalent_fractions_compute`, `compare_fractions_same_num`, `compare_fractions_unlike`, `simplify_fraction`. **Batch 2** (`ec69aae`): `improper_to_mixed`, `mixed_to_improper` (first use of `AnswerFormat.mixedNumber`), `sub_fractions_like_denom`, `add_fractions_unlike_denom`, `sub_fractions_unlike_denom` (LCM common denominator). **Batch 3** (`2484ece`): `mult_fraction_by_whole`, `mult_fractions_proper` + new `AreaGridSpec`/`AreaGrid` widget (three-tier shading: overlap = product, row/col = factors, empty = outside). **Batch 4** (`e0b40b1`): `div_unit_fraction_by_whole`, `div_whole_by_unit_fraction`, `div_fraction_by_fraction` (keep, change, flip), `fraction_as_division` (CCSS word-problem framing reusing the existing name + edible-item pools, constrained to X < Y so canonical stays proper). 15 short labels added to `build_catalog.dart`; registry regenerated (`74a0c9f`). **In-hand polish** (`ddb2808`, `3fb0660`): keypad's space key now visually renders as " and " (matching the prompt's "Write 1 and 3/4 as ..." phrasing) and is no-op on empty / trailing-space input — fixed an invisible-space footgun spotted on the emulator. Then the cleaner `AnswerShape` enum replaced the original boolean `requiresCanonicalForm`: shape values `any` / `exactString` / `mixedForm` / `improperFraction` let `improper_to_mixed` / `mixed_to_improper` accept simplified equivalents *within the required shape* (e.g. "3 1/2" for canonical "3 2/4") while still rejecting wrong-shape answers (e.g. typing the improper form when the lesson asked for mixed). **195 tests pass** (was 137 at session start; +58 across the slice). `flutter analyze` clean throughout.
- **Next action:** Pick the next sub-slice. Easy options remaining ([curriculum.md §5.1](curriculum.md)):
  - **#3b Mixed-number arithmetic + leftover proper-fraction concepts** (~7 more) — `add_mixed_like_denom`, `sub_mixed_like_denom`, `add_mixed_unlike_denom`, `sub_mixed_unlike_denom`, `mult_mixed_numbers`, `mult_as_scaling`, `whole_number_as_fraction`. Punted out of this slice. Natural follow-up; everything plumbs into the existing `AnswerFormat.mixedNumber` path.
  - **#5 Word-problem framework, continued** — multiplication contexts (`builds`, `bakes/makes`, `saves`). Needs a design call on concept ID since curriculum.md doesn't have `mult_word_problems_within_100` (per design principle 4 — most word-problem framings aren't broken out). Two paths: (a) embed multiplication word-problem questions inside `mult_facts_within_100` as a question-shape variant, or (b) add a new curriculum entry.
  - **#5b Two-step word problems** (`add_sub_2step_word_problems`) — also needs design (template-shape strategy for 2-step).
  - **#6 Rationals** — `rationals_add_sub`, `rationals_multiply_divide`. **Now unblocked** by the foundation — answer-equivalence + canonical reduction work for any rational, not just fractions.
  - Other categories untouched: decimals/percentages (#7), ratios (#8), geometry / area / perimeter (#9–11), pre-algebra / equations (#12–14), statistics (#16). All are fresh-context slices.
