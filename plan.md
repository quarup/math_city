...
- **Phase:** Phase 7 — City Builder: Simple DAG Proof. Building the persistent isometric city with the full *mechanics* of the design (multi-gate DAG, citizen bubbles, growth-from-mix) but a deliberately small content set (~10 buildings, ~5–10 hand-written beats); Phase 8 designs the full DAG, Phase 9 implements it. Phase 6 left two deliberately-deferred sub-tracks for a later return: (a) selective DeepMind expansion (variety only, optional); (b) adaptive-system + mix-policy tuning (needs playtest data first). Live counts for the curriculum are in [curriculum.md](curriculum.md).
...
- **Last action:** **Chunk 2 — first visible city UI landed (Flame isometric renderer + build-mode placement + "My City" hub navigation).** New **`lib/game/city/`** Flame layer: `IsoGrid` (pure 2:1-dimetric grid geometry — `centerOf` / `tileAt` round-trip, no Flutter/Flame imports so it's unit-tested), `CityBoardComponent` (`PositionComponent with TapCallbacks` — renders the grass-diamond terrain grid + placeholder extruded-box buildings colored by category with the building emoji on the top face, painter's-ordered by `col+row`; reports tile taps), and `IsoCityGame` (`FlameGame with ScaleCallbacks` — single-finger pan + two-finger pinch-zoom via `camera.viewfinder`, zoom-to-fit on first resize, position clamped to board+margin). **Presentation:** new `lib/presentation/city/city_screen.dart` — AppBar (player avatar+name, 🧱/🔬 balances), `GameWidget` body, a horizontal build-mode catalog `bottomNavigationBar` listing the player's `BuildingTypesResearched` (placeholder emoji + name + 🧱 cost; unaffordable types greyed at 0.4 opacity + unselectable; selected = primary-color border), and a "Play math" FAB → spin wheel. Tap a catalog card to select → tap a free tile to place (occupied / unaffordable / no-selection paths show a SnackBar). **State:** new `lib/state/city_provider.dart` — `activeCityProvider`, `placementsProvider`, `researchedBuildingsProvider` (registry order), and `cityActionsProvider` (`CityActions.placeBuilding` = spend 🧱 → insert row → invalidate placements/player/all-players). **Data:** `database.dart` gained `cityForPlayer`, `placementsForCity`, `researchedBuildingTypeIds`, and `placeBuilding` (spends via `incrementPlayerBricks(-cost)` so lifetime stays monotone; `placedAtRound` = current placement count as a monotonic stand-in until round tracking lands). **Navigation reshaped (per user, 2026-05-23):** the home player-chip now opens **My City** (the per-player hub) instead of going straight to the wheel; My City's FAB pushes the `SpinScreen`; the spin AppBar's leading `location_city` button pops back to My City. CityScreen is pushed with `RouteSettings(name: 'city')` and `result_screen._pushSpin` now collapses the spin→question→result loop back onto that city route (falling back to home) so a fresh spin always sits directly above My City. **Tests:** new `test/game/city/iso_grid_test` (full-board round-trip + bounding box + out-of-bounds), `test/data/city_placement_test` (city queries + placeBuilding spend/lifetime/round-counter), `test/presentation/city_screen_test` (CityScreen mounts, catalog shows the pre-researched mayor's office, Play FAB present). **All 632 tests pass; analyze clean (modulo the one pre-existing test-file line-length lint).**
- **Prior action:** **Chunk 1 — domain spine landed (Drift v7 + pure-Dart city domain + research-award wiring, all unit-tested; no UI).** Drift bumped to v7 with `Player.{brickBalance, lifetimeBricksEarned, researchBalance, lifetimeResearchEarned}` (rename of `currentStars`/`lifetimeStarsEarned` + new 🔬 pair) plus 5 new tables: `Cities` (per-player city instance — `(playerId, cityMapId)` so a player can own multiple cities as they unlock more maps), `BuildingPlacements`, `BuildingTypesResearched`, `ConceptBandMilestones` (research-award log; bandIndex is `int` into the in-code threshold list so adding a 3rd band needs no migration), and `StoryBeatStates`. Wipe-and-recreate migration. **Domain:** new `lib/domain/city/` with value types `BuildingType`, `UnlockRule` + `UnlockContext`, `StoryBeat`, `TriggerRule` + `TriggerContext`, `CityMap`, and `BuildingCategory` / `BeatTone` / `BeatKind` enums; registries `buildingRegistry` (the locked 10-building catalog — mayor's office free + ungated, single home `brickCost: 0` so the very first 🔬 unlocks an immediate placement, the other 8 at placeholder `brickCost: 10 / researchCost: 1` all gated on mayor's office), `beatRegistry` (2 sample beats — `demand_first_home` + `praise_first_home` — exercising the engine; full ~5–10 set in a follow-up chunk), `cityMapRegistry` (beginner town only — 12×12); engines `BuildingDagEngine` (returns available-to-research set; building-DAG, distinct from the existing math-concept `lib/domain/concepts/dag_engine.dart`) and `BeatEngine`. **Research-award rule** (`lib/domain/city/research_awards.dart`): `researchAwardThresholds = [0.5, 0.85]` const list + pure-Dart `newlyCrossedBands(oldP, newP, alreadyAwardedBandIndices)`. **Wiring:** `recordAnswer` in `proficiency_provider.dart` now reads `awardedBandIndicesFor`, calls `newlyCrossedBands`, and for each new index records a `ConceptBandMilestones` row + calls `incrementPlayerResearch(playerId, 1)` (idempotent on re-runs via the composite primary key). `AppDatabase.createPlayer` extended to auto-create the beginner `City` row + pre-research the mayor's office. **UI rename pass:** `currentStars`/`lifetimeStarsEarned`/`updatePlayerStars`/`totalStarsProvider`/`TotalStarsNotifier`/`starsForBand`/`starsEarned`/`pulseStars`/`_StarAward` and all dartdoc "stars" mentions renamed to the `bricks` shape. HomeScreen player chip now shows 🧱 + 🔬 balances side-by-side. **Tests:** new `test/domain/city/` directory with `unlock_rule_test`, `trigger_rule_test`, `dag_engine_test`, `beat_engine_test`, `building_registry_test`, `research_awards_test`; new `test/data/city_create_player_test` covers auto-create + `incrementPlayerResearch` + `recordBandMilestone` idempotency; new `test/state/proficiency_research_award_test` covers band-crossing awards end-to-end including the re-crossing-after-dip idempotency and the wrong-answer-no-award path. **All 618 tests pass; analyze clean (modulo the pre-existing test-file lint).**
- **Next action:** **Phase 7 Chunk 3 — research UI panel.** A panel (reachable from My City) listing buildings currently *available to research* per each type's `unlockRule` (evaluated by `BuildingDagEngine.availableToResearch` against the player's lifetime-🧱 / placed-buildings / population / fired-beats state); each row shows the 🔬 `researchCost`, name, category, and a one-line description. Tapping a row with sufficient 🔬 spends it, inserts a `BuildingTypesResearched` row (need a `researchBuilding` DB method mirroring `placeBuilding`'s spend-then-invalidate shape), and the building joins the build-mode catalog. Buildings not yet available-to-research are not listed (discovery-based — no visible tech tree). **Then Chunk 4** move-mode UI, **Chunk 5** auto-road generation + population growth model, **Chunk 6** beat-bubble overlay + the remaining ~5–10 beats. Phase 6 deferred work (selective DeepMind expansion, mix-policy + adaptive-system tuning, kid validation) revisited post-Phase 7. **Manual verification still owed (sandbox is headless — can't drive the Flame canvas):** on a device/emulator, confirm pan/zoom feel, tap-to-place hit accuracy across the iso grid, and that `TapCallbacks` taps aren't swallowed by `ScaleCallbacks` (fallback if they are: detect a tap inside `onScaleEnd` when the focal point barely moved).
...
BuildingType (static catalog — registry in code, mirrored in `city_builder.md §3`)
  ...
  unlockRule,                    // typed value: AND-combination of {minLifetimeBricks, requiredBuildingsPlaced[], minPopulation, requiredBeatsFired[]} — gates whether the building is *available to research*; once researched, it's permanently in the player's catalog (still subject to brickCost on each placement)
...
- **Unlock model is two-step:** a building is *available to research* iff its `unlockRule` (currency / prereq-building / population / beat AND-combination) evaluates true → player spends `researchCost` → row appears in `BuildingTypeResearched` → building is permanently in the player's build menu, subject only to `brickCost` per placement. This lets the player choose which available building to unlock rather than having unlocks auto-fire.
...
- **Phase 9:** Build a custom dataset starting from a free CC0 isometric kit (Kenney's City Kit etc.), then extend by prompting an image generator (Nano Banana / Stable Diffusion / similar) for new buildings in the same style. **Style consistency is the binding constraint** — feed the generator 2–3 anchor buildings as style references on every prompt so lighting / shadow direction / palette stay coherent across the catalog. Output is PNG-with-transparency from the same isometric perspective, with per-building footprint metadata (1×1 vs 2×2 tiles, anchor point so the base sits on the grid).

**Build:**
- [ ] Drift schema (v7): currency split on `Player` (brick/research balances + lifetime counters), plus new tables `City`, `CityMap`, `BuildingTypeResearched`, `BuildingPlacement`, `StoryBeatState`, `ConceptBandMilestone` — see Data Model section. Wipe-and-recreate migration (still pre-real-users per plan.md). Static catalogs (`BuildingType`, `StoryBeat`, `CityMap`) live as code registries, not Drift rows.
- [ ] One beginner `CityMap` definition: ~12×12 tile grid, fixed terrain
- [ ] Initial `BuildingType` registry (the ~10 from the catalog above), each with `category`, `brickCost`, `researchCost`, `populationContribution`, `serviceProvision`, `unlockRule`, and a placeholder `assetRef`
- [ ] **DAG unlock engine** (pure-Dart, well unit-tested) — a building's `unlockRule` is a typed value combining any of `{minLifetimeBricks, requiredBuildingsPlaced, minPopulation, requiredBeatsFired}` with AND semantics. Engine evaluates the rule against current city + player state and returns the *available-to-research* set; emits "newly available" events on transitions. **Note:** a building moving into the available-to-research set does NOT auto-unlock it — the player must still spend `researchCost` to add it to `BuildingTypeResearched`.
- [ ] **Research-award rule** (pure-Dart, well unit-tested) — wired into the proficiency-update path: on each `p` update, for every threshold in `researchAwardThresholds` that's been crossed (old `p` below, new `p` ≥) and has no `ConceptBandMilestone` row, insert the row and increment the player's research balance. Idempotent (re-running the update with the same inputs awards nothing).
- [ ] **Story-beat engine** (pure-Dart, well unit-tested) — each beat has trigger conditions (`buildingsPresent` / `buildingsAbsent` / `minPopulation` / `minBuildingAgeForX` / `requiredBeatsFired` / `minBricksEarnedSinceLastBeat`), emoji + sticker payload, and short + long text. Engine maintains a `(beatId → state)` map across sessions; "rotates" unack'd bubbles by hiding them after N rounds and re-firing later
...
- [ ] **Research UI** — separate panel listing buildings currently *available to research* (per `unlockRule`); each row shows the building's 🔬 `researchCost`, name, category, and a one-line description. Tapping a row with sufficient research balance spends 🔬, inserts a `BuildingTypeResearched` row, and adds the building to the build-mode catalog. Buildings that aren't yet available-to-research are not listed (discovery-based — no visible tech tree).
...
- [ ] Move-mode UI: pick up an existing building, tap a new free tile to drop it (no 🧱 cost)
