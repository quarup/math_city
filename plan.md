...
- **Next action:** Pick the next sub-slice. What's still missing ([curriculum.md §5.1](curriculum.md)):
  ...
  - **Data displays** — `BarChart` widget landed in Chunk 27 with `bar_graph_read` / `bar_graph_compare` / `scaled_bar_graph_read`. Natural follow-ups using the same widget shape: `picture_graph_read` + `scaled_picture_graph` (emoji/icon variant — would either extend the widget with a "stacked icons" rendering mode or be a sibling widget `PictureGraph`); `histogram` (extends `BarChart` with adjacent bars + numeric bin labels — likely a `HistogramSpec` sibling). Still needs its own widget: `dot_plot` / `line_plot_*` (`DotPlot`), `box_plot` / `compare_two_distributions` (`BoxPlot`), `scatter_plot_*` (`ScatterPlot` — could reuse `CoordinatePlane` with non-labelled points + larger range).
