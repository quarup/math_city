...
**Follow-ups noted, not done:** the starter pack and `pickNext` still sort *easiest first*, so a grade-3 player's first 12 concepts are all grade-2 keypad fluency checks (by design, but ~40 of them precede any grade-3 content); consider interleaving at-grade concepts. Review segments have no visual marker yet (only "NEW" does).
