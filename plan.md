...
- **Phase:** Phase 6 — Full Question Bank. **164 generators implemented**; broad K–8 coverage. Remaining gaps: coordinate plane, transformations, data displays (bar/box/histogram), compound probability, some 3D volume (cylinder/cone/sphere need π), most K–2 counting/place-value foundations.
- **Last updated:** 2026-05-17
- **Last action:** **Chunk 22 — per-table multiplication facts** — 9 generators via one parameterised helper. `multFactsN(int n)` in `mult_div_generators.dart` produces `n × b = ?` for `b ∈ [1, 10]`, tagged with the matching `mult_facts_$n` concept ID. Registry wires `mult_facts_2` through `mult_facts_10`. Lets the wheel surface a specific table the kid is drilling (e.g. their ×7s) without dragging in the others. Overrides: dropped `skip_count_2 / 5 / 10` (no generators) from `mult_facts_2 / 5 / 10` and `mult_meaning_groups` from `mult_facts_3`; `add_within_100` stands in as the basic arithmetic background. `mult_facts_4 / 6 / 7 / 8 / 9` need no override — their curriculum prereqs (other `mult_facts_N`) are now resolved by this chunk. Tests: +9 parameterised groups @ 200 + one boundary check; **344 total pass**, `flutter analyze` clean.
