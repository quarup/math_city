...
- **Phase:** Phase 6 — Full Question Bank. **150 generators implemented**; broad K–8 coverage. Remaining gaps: coordinate plane, transformations, data displays (bar/box/histogram), compound probability, some 3D volume (cylinder/cone/sphere need π), most K–2 counting/place-value foundations.
- **Last updated:** 2026-05-17
- **Last action:** **Chunk 20 — properties of operations + number theory cleanup** — 5 generators across three existing files plus one prereq override. Number theory: `distributive_with_gcf` (G6, "Use the GCF to rewrite 12 + 18 as a product" → "6 × (2 + 3)" — picks `g × p` and `g × q` with `gcd(p, q) = 1` so the inner sum is in lowest terms; misconception distractors include "pulled GCF but kept addends" and "raw sum"), `even_odd` (G2, "Is 14 even or odd?" MC over {even, odd, both, neither}, n ∈ [2, 99]). Add/sub: `equal_sign_meaning` (G1, "What goes in the box? 4 + 3 = ? + 2" → 5 — tests CCSS 1.OA.D.7 "equal sign means same value" with the LHS-total surfaced as a misconception distractor), `commutative_add` (G1, "If 8 + 5 = 13, what is 5 + 8?"). Mult/div: `commutative_mult` (G3, same shape as commutative_add). Override: dropped `skip_count_2` prereq from `even_odd` (no generator yet); substituted `sub_within_20` as the basic number-sense prereq. Tests: +5 groups @ 200–300; **329 total pass**, `flutter analyze` clean.
