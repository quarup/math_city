...
- **Phase:** Phase 6 — Full Question Bank. **140 generators implemented**; broad K–8 coverage. Remaining gaps: coordinate plane, transformations, data displays (bar/box/histogram), compound probability, some 3D volume (cylinder/cone/sphere need π).
- **Last updated:** 2026-05-17
- **Last action:** **Chunk 18 — Pythagorean + scientific notation ops + late-grade cleanup** — 5 generators in a new file `advanced_generators.dart` (kept separate because they span geometry, number theory, and rationals): `pythagorean_apply_2d` (G8, classic triples 3-4-5, 5-12-13, etc.; 70% ask hypotenuse / 30% ask leg), `volume_rect_prism_formula` (G5, l×w×h; override drops `volume_unit_cubes`), `scientific_notation_ops` (G8, multiplication: ab × 10^(p+q); distractor pool restructured with multiple fallbacks to handle the p=q=2 edge case where p·q == p+q), `compare_order_rationals` (G6, MC over 4 values from {integers, halves}; override substitutes `compare_decimals_thousandths`), `irrational_recognize` (G8, 14 curated items mixing rationals and irrationals; override substitutes `repeating_decimal_recognize`). Tests: +5 groups @ 300; **319 total pass**, `flutter analyze` clean.
- **Last action (prior):** **Chunk 17 — scientific notation + exp props + repeating decimals** — 5 generators. Commit `cf03d92`.
