...
- **Phase:** Phase 6 — Full Question Bank. **104 generators implemented**; five K–8 categories with coverage (`fractions` 26, `decimals_percent` 24, `ratios` 10/16, `expressions_equations` 10/14, `statistics` 4/15). Word problems + rationals + place value also covered. Geometry still untouched.
- **Last updated:** 2026-05-17
- **Last action:** **#16 Statistics (chunk 10) — entry** — new file `statistics_generators.dart` with 4 generators: `mean` (G6, "Find the mean: 4, 6, 8, 10" → 7; pick mean first then perturbations summing to 0 so the answer is always integer), `median` (G6, odd count 5 or 7 so the median is an exact middle value, no averaging step), `mode` (G6, generated with exactly one most-frequent value), `range_data` (G6, max − min). Prereq overrides for `median` (drop `compare_order_rationals`, substitute `sub_within_1000`) and `mode` (drop `classify_count_categories`, substitute `add_within_100`). Tests: +4 groups @ 300; **283 total pass**, `flutter analyze` clean.
- **Last action (prior):** **#12 Equations (chunk 9)** — `solve_two_step_eq`, `expand_linear_expression`, `add_subtract_linear_expressions`, `equivalent_expressions_props`, `substitute_to_check`. Commit `ab85038`.
...
- **Next action:** Pick the next sub-slice. Easy options remaining ([curriculum.md §5.1](curriculum.md)):
  - **#16 Statistics chunk 11** — `iqr` (Q3 − Q1), `mad` (mean absolute deviation). Both algorithmic.
