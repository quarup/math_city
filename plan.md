...
- **Next action:** **Phase-9 — keep filling out the building catalog (32/55 §3 buildings wired) + the remaining big features.** (1) **Generate + process art for the 6 artless wired buildings** — `mid_rise_apartment`, `sports_field`, `museum`, `stadium` (Phase-9 registry entries, no raws yet) and `grocery`, `park` (Phase-7 types, also raw-less) all render the box placeholder: generate NB raws per [prompts.txt](tools/sprite_pipeline/prompts.txt), run [process.py](tools/sprite_pipeline/process.py), set `numVariants`, run the §6 sync. (2) **Finish the remaining 23 of the 55 §3 buildings** the same way, generating + processing toward the full 97-variant catalog. (3) Bigger Phase-9 features still open: **themed maps + map switcher, building upgrade tiers, currency-funded events** (land expansion ✅ shipped). (4) **Variant cull** to §5.4 target counts still deferred (e.g. `single_home` has 6, target 4–5). (5) Land-expansion price ladder (🧱 60/150/300) is a designed-coherent placeholder — sanity-check it against real earn rates once playtested. **Gotcha hit this session:** iCloud/Finder ` 2`-suffixed duplicate-file pollution under `build/` + `.dart_tool/` broke the first Android build (`packJniLibs` zip conflict on `libsqlite3 2.so`); `flutter clean` + `find . -name '* 2.*' -delete` fixed it, recurs if the Documents folder re-syncs mid-build. **Phase-7 device-verification (non-sprite) still partly owed** — pan/zoom feel, tap-to-place accuracy, move-mode pick-up/drop highlight, citizen-bubble rotation/ack, demand-gated reveal, age-gated milestone bubble; most are now incidentally observable on the emulator. Once swept, run the **Phase-7 exit-criteria playthrough**. Phase 6 deferred work (selective DeepMind expansion, mix-policy + adaptive-system tuning, kid validation) revisited post-Phase-7. *(Variant-cull to §5.4 target counts deferred — all 16 pilot variants kept for now.)*
