...
- **Phase:** Phase 6 — Full Question Bank. **70 generators implemented**; two K–8 categories largely covered (`fractions` 26 + `decimals_percent` 14 of 26) plus solid word-problem coverage and Grade-7 rationals. Remaining decimals/percent (`percent_intro`, `percent_of_quantity`, repeating-decimal rows), ratios (#8), geometry, prealgebra, and statistics still untouched.
- **Last updated:** 2026-05-17
- **Last action:** **#7 Decimals sub-slice (chunk 2)** — 7 new generators appended to `decimal_generators.dart`: `decimal_to_thousandths_read` (G5), `compare_decimals_thousandths` (G5), `round_decimals` (G5, half-away-from-zero to whole/tenth/hundredth), `div_decimal_by_whole` (G5, generated as quotient × divisor for exact results), `div_by_decimal` (G6, quotient always a whole int 2..20), `decimal_to_fraction` (G5, AnswerFormat.fraction + exactString since lesson is "in lowest terms"; re-rolls to require non-trivial reduction), `fraction_to_decimal` (G6, AnswerFormat.decimal; denominators restricted to the terminating set {2, 4, 5, 8, 10, 20, 25, 50}). New helper `_wholeDistractors` for integer-answer distractor sets that need dedup-and-fallback. Short labels added; concept_registry regenerated (no prereq overrides needed — all chunk-2 prereqs already had generators). **249 tests pass** (was 239 at chunk-1 close; +10). `flutter analyze` clean. Commit-pending.
- **Last action (prior):** **#7 Decimals sub-slice (chunk 1)** — 7 new generators in a new file `lib/domain/questions/generators/decimal_generators.dart`:
...
- **Next action:** Pick the next sub-slice. Easy options remaining ([curriculum.md §5.1](curriculum.md)):
  - **#7 Decimals — chunk 3 (percent)** — `percent_intro` + `percent_of_quantity`. Needs a `PercentGrid` widget (or extension of `AreaGrid`, which currently only renders a row × col shaded rectangle — wrong shape for arbitrary N-out-of-100 shading). Could bundle `find_whole_from_part_percent` and `simple_interest` since they're algorithmic-only follow-ups. Will also remove the `decimals_fluent_4ops` override (now that all its prereqs are implemented) if any cleanup is needed.
