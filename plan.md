...
- **Last action:** **Phase 7 kickoff — design decisions locked, no code yet.** (1) **Currency model:** two-currency economy — 🧱 **bricks** (rename of `stars`; spent to place buildings; earned per correct answer) and 🔬 **research** (spent to unlock new building types; earned by per-concept band crossings — see *Domain Specs / Research-currency earning*). Number of award bands is configurable via a domain-level `List<double>` constant (v1 ships `[0.5, 0.85]`); `ConceptBandMilestone.bandIndex: int` keeps the schema future-proof for a 3rd band without a migration. (2) **Initial ~10 building catalog:** mayor's office / single home / apartment / school (civic-core+housing); clinic / power plant / waste management (services); grocery / coffee shop (commercial); park (entertainment). Hits all 4 categories so variety-multiplier + service-ratio mechanics have something to chew on. (3) **Placeholder art:** Phase 7 ships pure Flutter `CustomPainter` placeholders (colored isometric tiles + emoji + label per building type); `BuildingType.assetRef` is opaque so Phase 9's swap to PNGs is a registry change with zero domain churn. Phase 9 art pipeline = start from a free CC0 isometric kit (Kenney) and extend with image-gen (Nano Banana / SD) for new buildings in the same style. Plan.md + prd.md updated.
- **Next action:** **Phase 7 Chunk 1 — domain spine.** Drift v7 schema (currency split `Player.brickBalance` / `Player.researchBalance` + lifetime counters, plus the 5 new city-builder tables `City` / `CityMap` / `BuildingPlacement` / `ConceptBandMilestone` / `BuildingTypeResearched`); pure-Dart `BuildingType` + `StoryBeat` registries with the locked 10-building catalog stubbed in; pure-Dart `UnlockRule` + `TriggerRule` value types; pure-Dart `DagEngine` (returns the available-to-research set) and `BeatEngine` (returns the on-screen bubble set); pure-Dart research-award rule in the proficiency update path (wired through `ConceptBandMilestone` writes when band thresholds are crossed for the first time). All unit-tested. **No UI yet** — Phase 7's renderer + build-mode UI land in a later chunk. Phase 6 deferred work (selective DeepMind expansion, mix-policy + adaptive-system tuning, kid validation) will be revisited post-Phase 7.
