...
- **Phase:** Phase 6 — Full Question Bank. **174 generators implemented**; broad K–8 coverage. Remaining gaps: coordinate plane, transformations, data displays (bar/box/histogram), compound probability, some 3D volume (cylinder/cone/sphere need π), most K–2 counting/place-value foundations.
- **Last updated:** 2026-05-17
- **Last action:** **Chunk 24 — unknown-position + signed quantities + theoretical vs experimental** — 5 generators spread across four existing files. Add/sub: `missing_addend_within_20` (G1, "5 + ? = 13" → 8 — two shapes, sum-misconception distractor); `add_sub_unknown_position` (G1, "Find the missing number: a ± b = c with ? in any position" — six shapes covering both ops in each operand slot). Mult/div: `mult_1digit_by_multiple_of_10` (G3, "7 × 30 = ?" → 210; "forgot the zero" misconception surfaced). Integers: `signed_quantities_context` (G6, MC over real-world scenarios: "Which can be represented by a negative number?" — distractors are opposite-sign scenarios). Probability: `theoretical_vs_experimental` (G7, surfaces both theoretical 1/n and experimental successes/trials in one scenario and asks for one specifically; the other is the lead distractor). Override cleanups: `solve_one_step_eq_addition` tightened to `['missing_addend_within_20']` (the canonical curriculum prereq now lives). New override: `mult_1digit_by_multiple_of_10` drops the unimplemented `skip_count_10`, keeps `mult_facts_within_100`. Tests: +5 groups @ 200; **354 total pass**, `flutter analyze` clean.
- **Last action (prior):** **Chunk 23 — compare integers + powers of 10** — 5 generators. Commit `27bcac5`.
- **Last action (prior):** **Chunk 22 — per-table multiplication facts** — 9 generators. Commit `b16030b`.
- **Last action (prior):** **Chunk 21 — associative properties + missing-factor cleanup** — 5 generators. Commit `677c2ad`.
- **Last action (prior):** **Chunk 20 — properties of operations + number theory cleanup** — 5 generators. Commit `9afca37`.
