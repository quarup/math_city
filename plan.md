...
- **Phase:** Phase 6 — Full Question Bank. Catalog parser, multi-digit × ÷, full +/− word-problem coverage, place-value, rounding, and signed-integer arithmetic all shipped. **30 generators implemented** across all but two K–8 categories (decimals/percentages, ratios/proportions, geometry, prealgebra, statistics still untouched).
- **Last updated:** 2026-05-12
- **Last action:** Autonomous overnight session (3 commits on top of the morning's word-problem + debug-screen work). **Slice A** (commit `e471021`): extended `add_word_problems_within_100` to cover both addition and subtraction in one generator (the curriculum row is `+/−`, the morning shipped only `+`). Refactored `AdditionContext` → `WordProblemContext` with a `WordProblemOp` enum; added 3 subtraction contexts (`gives_away`, `eats`, `loses`); added an `edibleWordProblemItems` subset and a `requiresEdibleItems` flag so the `eats` context never picks bricks/cones; renamed function to `addSubWordProblemsWithin100`; bumped wheel shortLabel to `+− word`. **Slice B** (commit `3f818c0`): 6 new generators in [lib/domain/questions/generators/place_value_generators.dart](lib/domain/questions/generators/place_value_generators.dart) — `place_value_{2,3,multi}digit` and `round_to_10` / `round_to_100` / `round_multidigit_any_place`. Comma-formatted big numbers; single-digit answer pool with "wrong digit from same number" misconception bias. Landing `place_value_multidigit` let us **drop 4 transitive overrides** (`add_multidigit_standard_alg`, `sub_multidigit_standard_alg`, `mult_4digit_by_1digit`, `div_4digit_by_1digit` all now use their full curriculum.md prereqs). Added 3 new overrides for the new generators where canonical prereqs aren't implemented. **Slice C** (commit `574f967`): `integers_add`, `integers_subtract`, `integers_multiply_divide` in [lib/domain/questions/generators/integer_generators.dart](lib/domain/questions/generators/integer_generators.dart). Kid-textbook display conventions (parens around trailing-operand negatives). Implemented-grade ceiling moved G5 → G7. Rationals deferred until issue [#2](https://github.com/quarup/math_city/issues/2) is resolved (their answers are fractions; equivalence-checking matters there). **137 tests pass** (was 124 at session start; +13 across the 3 slices). `flutter analyze` clean throughout.
- **Next action:** Pick the next sub-slice. Easy options remaining ([curriculum.md §5.1](curriculum.md)):
  - **#3 Fraction generators (~14 more)** — extend the existing 4 with `equivalent_fractions_compute`, `compare_fractions_*`, `simplify_fraction`, `improper_to_mixed`/`mixed_to_improper`, `add_fractions_unlike_denom`, `sub_fractions_*`, `mult_fraction_by_whole`, `mult_fractions_proper`, `div_*`, `fraction_as_division`. **Resolve issue [#2](https://github.com/quarup/math_city/issues/2) (equivalent fractions accepted) as part of this slice** — many of these generators produce reducible answers, and ignoring equivalence will frustrate kids. Reuses `FractionBar`; one new `area_grid` widget needed for `mult_fractions_proper`.
  ...
  - **#6 Rationals** — `rationals_add_sub`, `rationals_multiply_divide`. Blocked on issue #2.
