...
- **Last action:** **Chunk 3 — research folded into the build catalog (no separate panel; design call 2026-05-24).** Rather than a standalone research screen, the bottom build-mode catalog now lists *both* researched buildings (placeable; 🧱 cost) and buildings currently *available to research* (locked; 🔬 cost + corner lock badge) in one scrolling row, so "research a building" and "place a building" share one surface. **Data:** `AppDatabase.researchBuilding` (spend 🔬 via `incrementPlayerResearch(-cost)` → insert `BuildingTypesResearched` row; idempotent on the composite PK so it never double-charges). **State:** `researchedBuildingsProvider` replaced by `cityCatalogProvider` → `List<CatalogEntry>` (`building` + `researched` flag), unioning `researchedBuildingTypeIds` with `BuildingDagEngine.availableToResearch` (its `UnlockContext` built from lifetime-🧱 + placed-building-type IDs; population/fired-beats stubbed at 0/∅ until Chunks 5–6), emitted in registry order so a card flips locked→placeable in place; new `CityActions.researchBuilding` spends then invalidates catalog+player+all-players. **UI:** `_CatalogCard` renders a locked entry with the 🔬 cost and an `Icons.lock` corner badge — visually distinct from the existing 0.4-opacity "can't afford 🧱" dim; tapping a locked card opens a confirm dialog ("Research X? Spend 🔬N…") before spending, then auto-selects the freshly-unlocked type for placement. Researched-but-unaffordable cards stay greyed/unselectable as before. **Tests:** `researchBuilding` group in `test/data/city_placement_test` (spend + lifetime-monotone + idempotency) and new `test/state/city_catalog_test` (fresh player = mayor only; placing the mayor surfaces the locked rest; a researched type flips to placeable). All 638 tests pass; analyze clean (modulo the pre-existing test-file lint).
- **Prior action:** **Chunk 2 — first visible city UI landed (Flame isometric renderer + build-mode placement + "My City" hub navigation).** New **`lib/game/city/`** Flame layer: `IsoGrid` (pure 2:1-dimetric grid geometry — `centerOf` / `tileAt` round-trip, no Flutter/Flame imports so it's unit-tested), `CityBoardComponent` (`PositionComponent with TapCallbacks` — renders the grass-diamond terrain grid + placeholder extruded-box buildings colored by category with the building emoji on the top face, painter's-ordered by `col+row`; reports tile taps), and `IsoCityGame` (`FlameGame with ScaleCallbacks` — single-finger pan + two-finger pinch-zoom via `camera.viewfinder`, zoom-to-fit on first resize, position clamped to board+margin). **Presentation:** new `lib/presentation/city/city_screen.dart` — AppBar (player avatar+name, 🧱/🔬 balances), `GameWidget` body, a horizontal build-mode catalog `bottomNavigationBar` listing the player's `BuildingTypesResearched` (placeholder emoji + name + 🧱 cost; unaffordable types greyed at 0.4 opacity + unselectable; selected = primary-color border), and a "Play math" FAB → spin wheel. Tap a catalog card to select → tap a free tile to place (occupied / unaffordable / no-selection paths show a SnackBar). **State:** new `lib/state/city_provider.dart` — `activeCityProvider`, `placementsProvider`, `researchedBuildingsProvider` (registry order), and `cityActionsProvider` (`CityActions.placeBuilding` = spend 🧱 → insert row → invalidate placements/player/all-players). **Data:** `database.dart` gained `cityForPlayer`, `placementsForCity`, `researchedBuildingTypeIds`, and `placeBuilding` (spends via `incrementPlayerBricks(-cost)` so lifetime stays monotone; `placedAtRound` = current placement count as a monotonic stand-in until round tracking lands). **Navigation reshaped (per user, 2026-05-23):** the home player-chip now opens **My City** (the per-player hub) instead of going straight to the wheel; My City's FAB pushes the `SpinScreen`; the spin AppBar's leading `location_city` button pops back to My City. CityScreen is pushed with `RouteSettings(name: 'city')` and `result_screen._pushSpin` now collapses the spin→question→result loop back onto that city route (falling back to home) so a fresh spin always sits directly above My City. **Tests:** new `test/game/city/iso_grid_test` (full-board round-trip + bounding box + out-of-bounds), `test/data/city_placement_test` (city queries + placeBuilding spend/lifetime/round-counter), `test/presentation/city_screen_test` (CityScreen mounts, catalog shows the pre-researched mayor's office, Play FAB present). **All 632 tests pass; analyze clean (modulo the one pre-existing test-file line-length lint).**
- **Next action:** **Phase 7 Chunk 4 — move-mode UI.** Generalize relocation to all placements (the `unique` mayor's office already relocates on re-place): a move toggle / long-press enters move mode, then drag-or-tap retiles an existing placement via the existing `moveBuildingPlacement` DB method, with occupied-tile rejection. **Then Chunk 5** auto-road generation + population growth model (feeds the so-far-stubbed `population` / service-ratio inputs that `cityCatalogProvider` currently passes as 0), **Chunk 6** beat-bubble overlay + the remaining ~5–10 beats (feeds the stubbed `firedBeatIds`). Phase 6 deferred work (selective DeepMind expansion, mix-policy + adaptive-system tuning, kid validation) revisited post-Phase 7. **Manual verification still owed (sandbox is headless — can't drive the Flame canvas):** on a device/emulator, confirm pan/zoom feel, tap-to-place hit accuracy across the iso grid, that `TapCallbacks` taps aren't swallowed by `ScaleCallbacks` (fallback: detect a tap inside `onScaleEnd` when the focal point barely moved), and that the new locked-card + research confirm-dialog flow reads clearly at kid scale.
