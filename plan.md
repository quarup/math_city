...
- **Phase:** Phase 6 — Full Question Bank. **184 generators implemented**; broad K–8 coverage. Remaining gaps: coordinate plane, transformations, data displays (bar/box/histogram), compound probability, some 3D volume (cylinder/cone/sphere need π), angle concepts (need `angle_diagram` widget), some kindergarten counting concepts.
- **Last updated:** 2026-05-17
- **Last action:** **Chunk 26 — K-2 add/sub cleanup** — 5 generators in `add_sub_generators.dart` plus one override cleanup. `ten_more_ten_less` (G1, "What is 10 more than 47?" → 57 — two directions; "changed the ones place" misconception surfaced). `add_2digit_1digit` (G1, "23 + 5 = ?" — sum capped at 99). `sub_multiples_of_10` (G1, "60 − 40 = ?" → 20). `add_up_to_4_2digit` (G2, 3 or 4 two-digit addends summing to ≤ 99). `mental_add_10_or_100` (G2, "What is 100 more than 247?" — two amounts × two directions). Override cleanups: `add_within_100` tightened to `['add_2digit_1digit']` (the canonical prereq now lives). New overrides: `ten_more_ten_less` drops `count_to_100_by_10` (no gen) → `place_value_2digit`; `sub_multiples_of_10` drops `skip_count_10` (no gen) → `sub_within_20`. Tests: +5 groups @ 200; **364 total pass**, `flutter analyze` clean.
- **Last action (prior):** **Chunk 25 — G8 algebra + triangle inequality + patterns** — 5 generators. Commit `51c5835`.
