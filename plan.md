...
- **Last action:** **Phase-9 art-pipeline tooling end-to-end on synthetic inputs.** Locked four art-pipeline design points + shipped the tooling under [tools/sprite_pipeline/](tools/sprite_pipeline/). **(1) 10m/tile scale** — a residential street (6m roadway + 2m sidewalk × 2) is exactly one tile; sanity-checked across the catalog (1×1 `single_home` = 10×10m lot, capstones intentionally stylised-small with iconic art cues carrying recognition rather than square-meterage). [city_builder.md §5.1](city_builder.md) **(2) 97 sprite variants** for the 55 §3 anchors (unique/capstones → 1, common mid-arc → 2, frequently placed → 3, highest-volume housing/parks/coffee shop → 4–5); Phase-9 wires this in via `BuildingType.numVariants` + `BuildingPlacement.assetVariantIndex` (additive). [city_builder.md §5.4](city_builder.md) **(3) "No text on sprites" + horizontal-flip-for-facing** — sprites all generated face south-west; renderer flips horizontally per placement to face the front toward the nearest road. No 4-facing rotation system (mobile-iso convention + 4× cost not justified); no camera rotation in v1. Forward-compatible if revisited. [city_builder.md §5.1 hard rule + §7 resolved 2026-06-04] **(4) [tools/sprite_pipeline/generate_prompts.py](tools/sprite_pipeline/generate_prompts.py)** → 55 Nano Banana prompts in [prompts.txt](tools/sprite_pipeline/prompts.txt) with `Nx` variant prefix + absolute meters footprint + strict (gcd-reduced) ratio + `No text.` suffix. Iterated the template several times in-session as I observed NB skewing rectangles toward square (added strict ratio) and putting signs on shops (added no-text rule to unlock the flip-for-variety mechanic). **(5) [tools/sprite_pipeline/process.py](tools/sprite_pipeline/process.py)** → load raw NB PNG → rembg background removal → non-transparent bounding-box crop → resize to per-footprint canvas at **TILE_W=192px** (matches Flame's `tileWidth=64 × maxZoom=3` from [iso_grid.dart](lib/game/city/iso_grid.dart) so sprites stay sharp at every supported zoom) → paste at bottom-center anchor (south corner of lot diamond) → pngquant compress → emit debug overlay with the tile diamond drawn over the sprite for visual QA. End-to-end verified in the web sandbox on synthetic 1×1 (→192×240 final) and 3×1 (→384×480 final) inputs. **(6) [city_builder.md §5.2](city_builder.md) alternatives-considered research log** documents the rejected paths: CC0 vector pack only (Kenney Isometric Vector Buildings covers only ~25–30 of catalog), mix multiple CC0 packs (style seams visible to kids), procedural `CustomPainter` (Phase-7 placeholders read flat), freelance artist ($1.5–3k Phase-7 / $8–20k Phase-9 indie — out of budget for a free hobby project), Recraft V4 and Magnific (first-hand-evaluated and underwhelming), raster→vector tracing (messy paths, painterly mismatch). **No game code touched.**
- **Prior action:** **Phase-9 building-art pipeline locked — Nano Banana, style-anchored to hand-curated CC0 reference images.** Single source for all 55 §3 buildings; same 2–3 anchor refs (from the [Kenney "Isometric Tiles" series](https://kenney.nl/assets/tag:isometric)) fed on every prompt for cross-catalog style coherence. Raster PNG-with-transparency at 2:1 dimetric. Three alternatives rejected first-hand: procedural `CustomPainter` buildings (Phase-7 placeholders read flat), mixing finished sprites from multiple CC0 packs (visible style seams), and the native-vector GenAI tools (Recraft V4, Magnific — unimpressive at the quality bar we want). Two further alternatives rejected on cost/coverage: commissioning a freelance artist (~$8–20k indie for 55 buildings) and a CC0-only vector path (Kenney Isometric Vector Buildings covers only ~25–30 of the catalog). **Docs:** new [city_builder.md §5.1](city_builder.md) (pipeline) + §5.2 (alternatives-considered research log); plan.md Locked Decisions row added; Phase-7 "Art roadmap" + Phase-9 "Building art pipeline" bullet updated to point at the new decision. No code changes this session.
- **Earlier action:** **Phase 8 first draft — [city_builder.md](city_builder.md) authored (§1–§7).** Content-only, no code. **§1 References & Lessons** from five reference games (SimCity 4 RCI/reward-buildings, Cities: Skylines milestone-XP pacing, Anno need-driven tier-ups, SimCity BuildIt as the cautionary tight-economy case, cozy-game loops) with explicit borrow/reject lists — the throughline: progression is earned by *learning math*, never grind/timers, and imbalance only ever *slows* growth (no kid fail-states). **§2 Categories** maps the four `BuildingCategory` roles onto the existing growth model (no mechanics change). **§3 Building catalog** fully specs **55 anchor buildings** across four within-category arcs (civic+housing 13, services 15, commercial 13, entertainment 14) — costs (🧱/🔬), pop/service profiles, realistic footprints (`1×1`→`6×6`, incl. rectangular `3×1`/`4×2`/`4×3`; towers compact-but-tall, capstones sprawl), full typed `unlockRule`s, rooted at `mayors_office`; §3.5 economy sanity-check, §3.6 long-tail patterns, §3.7 DAG sanity (no cycles; multi-parent nodes justified). **§4 Beat catalog** — 73 beats: 54 demand (one per non-starter building, doubling as the `requiredBeatsRead` discovery gate), 15 praise, 2 ratio-warnings, milestone/recurring; cross-checked so every `reads:` resolves. **§5 Asset checklist** (CC0/CC-BY hybrid pipeline). **§6 status** (Phase-7 ✅ rows only). **§7 open questions.** Three structure decisions locked with the user: education→`services`, `water`=hard-gating, 7-rung housing spine. Extends the Phase-7 model unchanged (two currencies, four categories, service-ratio+variety growth, typed rules). Two small additive Phase-9 mechanic needs flagged: multi-tile/non-square footprint rendering/placement (up to `6×6`), and `lopsided`/`growthStalled` booleans in `TriggerContext` for the two ratio-warning beats; plus land expansion is now a late-game prerequisite (capstones don't fit the `12×12` beginner map). **Awaiting user review of the draft.**
- **Next action:** **Phase-9 art-pipeline pilot — process the first real Nano Banana sprites + wire a small set into the game to shake out the kinks.** (1) Save the first batch of real NB outputs as `tools/sprite_pipeline/raw/<id>_v<n>.png` (some already exist from in-session prompt tuning: `coffee_shop_v1`, `mayors_office_v1`, `townhouse_row_v1`, `observation_tower_v1`, `amusement_park_v1` — push these from the local machine). (2) Install deps locally — `pip install Pillow "rembg[cpu]"` + `pngquant` (apt / brew / etc., per [tools/sprite_pipeline/README.md](tools/sprite_pipeline/README.md)). (3) Run `python3 tools/sprite_pipeline/process.py tools/sprite_pipeline/raw/*.png` and open the `tools/sprite_pipeline/debug/` overlays: does the sprite's south corner sit on the red diamond's south corner? Re-roll or hand-nudge any outliers — the `townhouse_row` asymmetric-sidewalk case is the most-likely flag. (4) **First Phase-9 game-code change:** swap the Flame renderer's `CustomPainter` placeholder for those building IDs to load `assets/buildings/<id>_v<n>.png` via the existing opaque `assetRef` (a resolver change, no domain churn — see *Data Model* note on `assetRef`). Pick variant deterministically per placement (e.g. seeded by `gridX * 1000 + gridY`) until the full `BuildingPlacement.assetVariantIndex` column lands later in Phase 9. (5) Validate on device: do the sprites read at the right scale, do they tile alongside the remaining placeholder buildings without jarring style seams, does the bottom-center anchor place them correctly on their tile clusters. Tune `TILE_W` or `TOWER_HEADROOM` if needed. (6) Then expand to the full Phase-7 ten, then the rest of the 97. **Separately, Phase 7 device-verification (Flame canvas) still owed** — needs a real device/emulator (not available in the web sandbox); see *Deferred*. All headless Phase-7 work is complete (DAG unlocks, research/build/move UI, road gen, placement invariant, population growth, story beats + bubbles, the round clock). **Manual verification owed:** pan/zoom feel, tap-to-place hit accuracy, `TapCallbacks` vs `ScaleCallbacks` (fallback: detect a tap inside `onScaleEnd` when the focal point barely moved), move-mode pick-up/drop highlight, roads visible/correct, the locked-card + research confirm-dialog flow, population climbing as the kid plays, citizen bubbles appearing / rotating off / lingering after read, the demand-gated reveal (a building's locked card only appears after its demand bubble is opened), and the age-gated milestone bubble after ~10 rounds. Once verified on device, run the **Phase-7 exit-criteria playthrough**. Phase 6 deferred work (selective DeepMind expansion, mix-policy + adaptive-system tuning, kid validation) revisited post-Phase 7.
