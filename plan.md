...
- **Next action:** **Phase 7 remainder — all device-verification-heavy (Flame canvas; the headless sandbox can't drive it).** **Chunk 4** move-mode UI (toggle / long-press → drag-or-tap retiles an existing placement via the existing `moveBuildingPlacement`, occupied-tile rejection). **Chunk 5 (rest)** auto-road generation (roads auto-connect placed buildings, recompute on place/move, render under buildings) + a visible population counter on the city screen (the value is already live in `Cities.population`). **Chunk 6** beat-bubble overlay (emoji bubbles above buildings, tap-to-expand, ≤5, rotate unack'd) + the remaining ~5–10 beats (feeds the still-stubbed `firedBeatIds`). Phase 6 deferred work (selective DeepMind expansion, mix-policy + adaptive-system tuning, kid validation) revisited post-Phase 7. **Manual verification owed:** pan/zoom feel, tap-to-place hit accuracy, `TapCallbacks` vs `ScaleCallbacks` (fallback: detect a tap inside `onScaleEnd` when the focal point barely moved), the locked-card + research confirm-dialog flow, and that population visibly climbs as the kid plays math. *(If a headless-testable slice is wanted before device work: author the remaining Chunk-6 beats as data + `BeatEngine` unit tests — the engine already exists.)*
