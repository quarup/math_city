...
- **Phase:** Phase 6 — Full Question Bank. Priority items #3b, #5, #5b, #6 all landed in one session. Now **56 generators implemented**; one K–8 category fully covered (`fractions`, 26 generators) plus solid word-problem coverage and Grade-7 rationals. Decimals/percentages (#7), ratios (#8), geometry, prealgebra, and statistics still untouched.
- **Last updated:** 2026-05-13
- **Last action:** Four sub-slices, one session. **#3b mixed-number arithmetic** (commit `177873e`): 7 new fraction generators — `whole_number_as_fraction`, `add_mixed_like_denom`, `sub_mixed_like_denom`, `add_mixed_unlike_denom`, `sub_mixed_unlike_denom`, `mult_mixed_numbers`, `mult_as_scaling`. All plumb into the existing `AnswerFormat.mixedNumber` path; `mult_as_scaling` uses `AnswerFormat.string` with a 4-option MC pool ("bigger" / "smaller" / "the same" / "can't tell"). **#5 multiplication word problems** (commit `9162047`): `mult_compare_word` (Grade 4) — chose curriculum.md's existing `mult_compare_word` row over inventing a new `mult_word_problems_within_100` (design principle 4). Implemented algorithmically despite curriculum tagging it as dataset. Misconception distractor: k+n. **#5b two-step word problems** (commit `561f02d`): `add_sub_2step_word_problems` (Grade 2) — composes two contexts from existing `addSubContextsV1` pool, retries operand draws until intermediate + final ∈ [2, 100]. Edibility carries over between the two steps. **#6 rationals** (this commit): new `rationals_generators.dart` file with `rationals_add_sub` + `rationals_multiply_divide` (Grade 7). Forces at least one negative operand (purely positive cases already covered by the Grade-5 fraction generators). Kid-textbook trailing-negative parens style. Sign-flip + flipped-op misconception distractors. Prereq overrides drop `add_decimals` / `mult_decimals` (no generators yet). Short labels for all 11 new generators added to `build_catalog.dart`; registry regenerated. **209 tests pass** (was 195 at session start; +14 across the four sub-slices). `flutter analyze` clean throughout.
- **Next action:** Pick the next sub-slice. Easy options remaining ([curriculum.md §5.1](curriculum.md)):
  - **#7 Decimals / percentages** — `decimal_notation_tenths`, `decimal_notation_hundredths`, `compare_decimals_*`, `add_decimals`, `sub_decimals`, `mult_decimals`, `div_decimals`, `fraction_to_decimal`, `percent_intro`, `percent_of_quantity`. Unblocks the dropped prereq overrides on the rationals (decimals can come back into the DAG).
  - **#8 Ratios / proportions** — `ratio_equivalent`, `unit_rate`, `solve_proportion`. Smaller surface, no diagrams needed.
