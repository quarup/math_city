...
- **Last action:** **Chunk 6 (partial) — story beats authored + firing/persistence wired (bubble overlay UI still pending).** **Catalog:** grew `lib/domain/city/beat_registry.dart` to **9 hand-written beats** (housing demand/praise, pre-unlock clinic/power/waste service demands — the waste one a `minPopulation`-gated "trash" warning, a recurring brick-spaced "another park?" demand, grocery + coffee praise, an aged-mayor milestone), exercising every `TriggerRule` field. **Firing:** new `StoryBeatStates` DB helpers (`storyBeatStatesForPlayer`, `firedBeatIds`, `recordBeatFired` = set on-screen + bump fireCount + stamp lifetime-🧱 for brick spacing, `setBeatState` for dismiss/ack); `CityActions.fireBeats` evaluates `BeatEngine` against live city + beat state and fires any newly-eligible beat (skips ones already on-screen), ticked from each placement and each `recordAnswer`; new `onScreenBeatsProvider` exposes the bubble set for the UI; `cityCatalogProvider` now feeds real `firedBeatIds` (forward-looking — no v1 building gates on beats yet). **Deferred (needs a per-session round counter):** `minBuildingAgeForId` evaluation (age map passed empty, so the one age-gated beat stays dormant) and round-based bubble rotation. **Tests:** `beat_engine_test` + `beat_registry_test` (catalog) and new `test/state/beat_firing_test` (DB helpers; fire-on-place; no re-fire while on-screen; demand→praise handoff; brick-spaced re-fire after dismissal; empty-city no-op). All 680 tests pass; analyze clean.
- **Prior action:** **Chunk 5 — population growth model v1, now wired into the city.** Pure model `lib/domain/city/population_model.dart`: `populationCapacity(placed)` = housing `sum(populationContribution)` capped by the thinnest *gating* service (`{power, clinic, waste}`, each with a 20-resident free allowance so a hamlet survives infra-free; `school` is non-gating), then scaled by a desirability multiplier (`+0.05` per distinct variety type, capped `1.5`, ×`0.8` lopsidedness penalty when commercial+entertainment buildings outnumber housing >2:1); `stepPopulation(current, capacity, {rate=0.25})` ticks toward capacity (grow / shrink / hold, exact landing, no overshoot). All numbers are Phase-7 placeholders (Phase 8/9 tunes them). **Wiring:** new DB `setCityPopulation`; `CityActions.tickPopulation` recomputes capacity from the city's placements, steps the stored population once, persists, and invalidates `activeCityProvider` (so `cityCatalogProvider` — which gates unlocks on population — rebuilds). Ticked on each placement (immediate feedback) and from `ProficiencyNotifier.recordAnswer` (playing math grows the city). `cityCatalogProvider`'s `population` input is now live — only `firedBeatIds` stays stubbed (Chunk 6). **Tests:** `test/domain/city/population_model_test` (20 cases) + new `test/state/city_population_test` (persist; grow-to-capacity-then-hold; monotonic-no-overshoot; place-steps-pop; empty-city no-op). All 663 tests pass; analyze clean.
- **Next action:** **Phase 7 remainder — now mostly device-verification-heavy (Flame canvas).** **Chunk 4** move-mode UI (`moveBuildingPlacement` + the **placement-validation road-access invariant**: free placement on any unoccupied tile, but reject anything that would box a building in — two-way, covering both the placed/moved building and any neighbor it would seal off; walks the `footprint` perimeter so a future 2×2 is handled — superseding the bare occupied-tile check). **Chunk 5** ~~auto-road generation~~ (done — "full street grid", `road_network.dart`; **device-verification owed**) + a visible population counter still pending (value live in `Cities.population`). **Chunk 6** emoji-bubble overlay — draw `onScreenBeatsProvider` over the city (≤5, tap-to-expand `longText`, tap-to-dismiss → `setBeatState`). **Also pending (a mostly-headless prerequisite):** a per-session **round counter** — once it lands, stamp `placedAtRound` with it (replacing the placement-count stand-in), wire `maxBuildingAgeByTypeId` so the age-gated beat fires, and add round-based bubble rotation (hide unack'd after N rounds). Phase 6 deferred work (selective DeepMind expansion, mix-policy + adaptive-system tuning, kid validation) revisited post-Phase 7. **Manual verification owed:** pan/zoom feel, tap-to-place hit accuracy, `TapCallbacks` vs `ScaleCallbacks` (fallback: detect a tap inside `onScaleEnd` when the focal point barely moved), the locked-card + research confirm-dialog flow, population climbing as the kid plays, and bubbles appearing / dismissing.
...
- [ ] Move-mode UI: pick up an existing building, tap a new free tile to drop it (no 🧱 cost). Subject to the placement-validation invariant below
...
- [x] **Auto-road generation — "full street grid"** — `lib/domain/city/road_network.dart` (pure-Dart, 6 tests in `test/domain/city/road_network_test.dart`). `generateRoads` paves every empty tile within the built-up area (bounding box of all building footprints, expanded one tile and clamped to grid), so buildings sit in street-framed blocks while the untouched frontier stays green and the paved area grows with the city. A lone building gets a one-tile road frame; gaps between buildings fill in; the box's outer ring holds no buildings so the network is connected in the normal case. Rendered as grey tiles in the board's terrain pass (`CityBoardComponent.roads` + `IsoCityGame.setRoads`, same pending-buffer pattern as buildings), so buildings always draw on top. Recomputed in `CityScreen.build` from the live placements (so it refreshes on place + move). *Device-verification owed: roads visible/correct on the Flame canvas.*
