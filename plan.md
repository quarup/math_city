...
- **Phase:** Phase 6 — Full Question Bank. **158 generators implemented**; broad K–8 coverage. Remaining gaps: coordinate plane, transformations, data displays (bar/box/histogram), compound probability (needs `tree_diagram` widget), some 3D volume (cylinder/cone/sphere need π).
...
- **Next action:** Pick the next sub-slice. What's still missing ([curriculum.md §5.1](curriculum.md)):
  ...
  - **Remaining geometry** — `circumference_circle`, `area_circle` (also π). Angle relationships landed in Chunk 22: `supplementary_angles`, `complementary_angles`, `vertical_angles`, `triangle_angle_sum`, `parallel_lines_transversal`.
