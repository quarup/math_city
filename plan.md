...
- **Last action:** **Round clock — persistent per-player round counter, now driving building age + bubble rotation.** New `Players.roundsPlayed` column (Drift **v8**, wipe-and-recreate) — a monotonic count of answered questions that survives restarts (the "round" clock the city builder keys off). `ProficiencyNotifier.recordAnswer` bumps it once per answered question, *before* the population tick + beat eval so both see the fresh value (new DB helper `incrementRoundsPlayed`, returns the new value). `placeBuilding` now stamps `placedAtRound` with the player's live clock (was a placement-count stand-in), and `CityActions.fireBeats` builds the real `maxBuildingAgeByTypeId` map (each type's *oldest* instance = clock − min `placedAtRound`), so the age-gated `praise_established_town` milestone (mayor's office ≥10 rounds old + prior praise) now fires. `recordBeatFired` stamps `lastFiredAtRound`; `onScreenBeatsProvider` rotates an un-acknowledged bubble off screen once it's `kBubbleRotationRounds` (8) old, leaving its DB state `onScreen` so it doesn't re-fire and re-clutter. **Tests:** rewrote the `placedAtRound` test for clock semantics + 3 new in `test/state/beat_firing_test` (`incrementRoundsPlayed` monotonic/persist; age-gated beat holds at 5 rounds then fires at 10; bubble rotates off after the window). All 708 tests pass; analyze clean.
- **Prior action:** **Chunk 6 — story beats authored + firing/persistence wired.** **Catalog:** grew `lib/domain/city/beat_registry.dart` to **9 hand-written beats** (housing demand/praise, pre-unlock clinic/power/waste service demands — the waste one a `minPopulation`-gated "trash" warning, a recurring brick-spaced "another park?" demand, grocery + coffee praise, an aged-mayor milestone), exercising every `TriggerRule` field. **Firing:** new `StoryBeatStates` DB helpers (`storyBeatStatesForPlayer`, `firedBeatIds`, `recordBeatFired`, `setBeatState`); `CityActions.fireBeats` evaluates `BeatEngine` against live city + beat state and fires any newly-eligible beat (skips ones already on-screen), ticked from each placement and each `recordAnswer`; `onScreenBeatsProvider` exposes the bubble set for the UI; `cityCatalogProvider` feeds real `firedBeatIds`. **Emoji-bubble overlay** (`_CitizenBubbleOverlay`): ≤5 stickers, tap-to-expand `longText`, "Got it" → `CityActions.dismissBeat`; screen-space, not yet per-building-anchored (a Phase 9/polish refinement). **Tests:** `beat_engine_test` + `beat_registry_test` (catalog) + `test/state/beat_firing_test` (DB helpers; fire-on-place; no re-fire while on-screen; demand→praise handoff; brick-spaced re-fire after dismissal; empty-city no-op).
- **Next action:** **Phase 7 remainder — now device-verification-only (Flame canvas), which needs a real device/emulator (not available in the web sandbox).** All headless Phase-7 work is complete (DAG unlocks, research/build/move UI, road gen, placement invariant, population growth, story beats + bubbles, and the round clock). **Manual verification owed:** pan/zoom feel, tap-to-place hit accuracy, `TapCallbacks` vs `ScaleCallbacks` (fallback: detect a tap inside `onScaleEnd` when the focal point barely moved), move-mode pick-up/drop highlight, roads visible/correct, the locked-card + research confirm-dialog flow, population climbing as the kid plays, citizen bubbles appearing / rotating off / dismissing, and the age-gated milestone bubble after ~10 rounds. Once verified on device, run the **Phase-7 exit-criteria playthrough**. Phase 6 deferred work (selective DeepMind expansion, mix-policy + adaptive-system tuning, kid validation) revisited post-Phase 7.
