...
- **Phase:** Phase 6 — Full Question Bank. **153 generators implemented**; broad K–8 coverage. Remaining gaps: coordinate plane, transformations, data displays (bar/box/histogram), compound probability (needs `tree_diagram` widget), some 3D volume (cylinder/cone/sphere need π).
- **Last updated:** 2026-05-17
- **Toolchain in the web sandbox:** The repo ships a `SessionStart` hook ([.claude/hooks/session-start.sh](.claude/hooks/session-start.sh)) that installs **Flutter 3.41.7** (same pin as CI) and runs `flutter pub get` automatically at the start of every Claude-Code-on-the-web session. **`flutter`, `dart`, `flutter analyze`, and `flutter test` are available** — always verify changes locally before committing; don't push code unseen and "rely on CI" as a substitute. The hook runs in async mode, so on a cold start there's a ~1–2 minute window where Flutter is still downloading; if a `flutter` command fails immediately at session start, wait a moment and retry once.
- **Last action:** **Chunk 21 — G8 linear-equation & function family** — 6 generators across two files. In `equation_generators.dart`: `solve_linear_eq_with_distrib_collect` (G8, "Solve: 2(x + 3) + 4x = 30"; form a(x+b) + cx = d with integer x ∈ [2,9]; misconception distractors include "divided d straight by (a+c) ignoring the constant" and "ignored the cx term"), `solve_system_substitution` (G8, both eqns in slope-intercept; back-builds two distinct non-zero-slope lines through a picked integer (x, y); answer is ordered-pair string; distractor pool: swap coords, sign-flip x, sign-flip y), `solve_system_elimination` (G8, standard form +by/−by so adding eliminates y; c1 and c2 both `_signed()` since c2 can be negative). New file `function_generators.dart`: `slope_from_two_points` (G8, integer slopes in {-3..-1, 1..4}; misconceptions: wrong sign, gave Δy directly, gave Δx directly), `linear_function_construct` (G8, "Find the line through (x1,y1) and (x2,y2)" → "y = mx + b" string; eqn() helper handles m=±1, b=0 special-case rendering; distractor fallback list of intercept-tweak forms guarantees 3 distinct even at degenerate (0,0) cases), `function_definition_check` (G8, "Is this a function? {…}" Yes/No over sets of 3 or 4 (x,y) pairs; non-function case picks one x to repeat with a different y). Prereq overrides added: `slope_from_two_points → proportional_relationship` (drops the unimplemented `graph_proportional_slope` which needs the CoordinatePlane widget), `function_definition_check → linear_function_construct` (drops `graph_linear_equation` for the same reason). Following the geometry-generators convention, slope_from_two_points is implemented text-only despite the catalog marking `algorithmicWithDiagram` — visual CoordinatePlane widget can wire in later. Tests: +7 groups @ 300 iterations each (3 in `equation_generators_test.dart`, 4 in new `function_generators_test.dart`). All 333 tests pass; `flutter analyze` clean.
...
- **Next action:** Pick the next sub-slice. What's still missing ([curriculum.md §5.1](curriculum.md)):
  ...
  - **Remaining geometry** — `circumference_circle`, `area_circle` (also π). Angle relationships (`complementary_supplementary`, `vertical_angles`, `parallel_lines_transversal`) — algorithmic text-only OK, diagram-bearing variants need an `Angle` / `IntersectingLines` widget.
