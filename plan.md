...
- **Phase:** Phase 6 — Full Question Bank. Catalog parser shipped, multi-digit × ÷ shipped, and the word-problem framework + first 1-step generator are now shipped (priority-list #5).
- **Last updated:** 2026-05-12
- **Last action:** Shipped Phase 6 path #5 — word-problem framework + first generator. New subdir [lib/domain/questions/word_problems/](lib/domain/questions/word_problems/): `word_problem_framework.dart` (25 culturally-balanced names, 20 plural items including 4 city-builder-themed — bricks/paint cans/traffic cones/road signs, 3 v1 addition contexts — `collects`/`is_given`/`buys` — and a `composeAdditionWordProblem` helper), and `word_problem_generators.dart` with `addWordProblemsWithin100`. Quantities are constrained to a, b ≥ 2 so item nouns stay plural (no singular handling in v1). Subjects are referred to by repeated name rather than pronoun, sidestepping gender encoding and verb-conjugation traps. Misconception distractor: `a − b` (operation confusion). Wired into `GeneratorRegistry`; parser shortLabel set to `+ word`. No `_prereqOverrides` entry needed — both prereqs (`add_within_100`, `sub_within_100`) are already implemented. **119 tests pass** (was 111; +8 new in `test/domain/questions/word_problems_test.dart` covering pool invariants, composer placeholder substitution, end-to-end generator structure × 300 iterations, and pool-coverage saturation across 1000 iterations); `flutter analyze` clean. Also shipped a kDebugMode-only "Generator preview" debug screen ([lib/presentation/debug/concept_debug_screen.dart](lib/presentation/debug/concept_debug_screen.dart)) that lets us play any implemented generator against either MC or keypad without touching player state — used to play-test this generator on the Android emulator. Two play-test issues filed: [#1 scratch pad](https://github.com/quarup/math_city/issues/1), [#2 equivalent fractions not accepted](https://github.com/quarup/math_city/issues/2).
- **Next action:** Pick the next sub-slice. Easy options remaining in priority order ([curriculum.md §5.1](curriculum.md)):
  - **#5 Word-problem framework, continued** — extend with: multiplication contexts (#11 builds, #9 bakes/makes, #10 saves) wired into a new `mult_word_problems_*` generator; subtraction contexts (#4 gives away, #5 eats/uses up, #6 loses) wired into `sub_word_problems_within_100`; then 2-step (`add_sub_2step_word_problems`) once the 1-step shapes are validated.
  - **#3 Fraction generators** — extend the existing 4 to add `equivalent_fractions_compute`, `compare_fractions_same_num`, `compare_fractions_unlike`, `whole_number_as_fraction`, `simplify_fraction`, `improper_to_mixed`/`mixed_to_improper`, `add_fractions_unlike_denom`, `sub_fractions_*`, `mult_fraction_by_whole`, `mult_fractions_proper`, `div_unit_fraction_by_whole`, `div_whole_by_unit_fraction`, `div_fraction_by_fraction`, `fraction_as_division`. Reuses the existing `FractionBar` widget; some entries need a new `area_grid` widget (`mult_fractions_proper`).
  - **#6 Signed-number arithmetic** — `integers_add`, `integers_subtract`, `integers_multiply_divide`, `rationals_add_sub`, `rationals_multiply_divide`. Optional `NumberLine` diagram (already exists). Pure algorithmic.
  - **#15 Place-value / rounding** — `place_value_2digit`, `place_value_3digit`, `place_value_multidigit`, `round_to_10`, `round_to_100`, `round_multidigit_any_place`, `expanded_form_3digit`. Each unlocks downstream generators (e.g. landing `place_value_multidigit` lets us drop two prereq overrides). Strategic if we want broad K–4 coverage.
...
- [x] **Word-problem framework v1** in [lib/domain/questions/word_problems/](lib/domain/questions/word_problems/): 25 culturally-balanced names, 20 plural items (4 city-builder-themed: bricks, paint cans, traffic cones, road signs), 3 v1 addition contexts (`collects`, `is_given`, `buys`), `composeAdditionWordProblem` template helper. First generator: `add_word_problems_within_100` (sum ≤ 100, both operands ≥ 2 so item nouns stay plural). Subjects referred to by repeated name rather than pronoun — sidesteps gender encoding and verb conjugation traps with gender-neutral names. Misconception distractor: `a − b` (operation confusion).
- [ ] Extend the framework: subtraction contexts (`gives away`, `eats / uses up`, `loses`) → `sub_word_problems_within_100`; multiplication contexts (`builds`, `bakes / makes`, `saves`) → `mult_word_problems_within_100`; then 2-step (`add_sub_2step_word_problems`) once 1-step shapes are validated
