...
- **Phase:** **Phase 10 — Construction Loop (in progress; domain + schema + routing + city-screen sites device-verified 2026-09-20)** — spec in [city_builder.md §8](city_builder.md). Phase 9 is effectively complete for content (catalog + art + land); its remaining themed-maps item stays open, its upgrade-tiers and events items are superseded/deferred by Phase 10, and the price-tuning-by-playtest next action carries into Phase 10 (prices are now the *total* a site is paid down to). Phase 9 as it stood: Rich Implementation & Graphics (in progress). Implementing the [city_builder.md](city_builder.md) catalog: **all 55 §3 buildings wired and sprite-backed** (added `gym` — services, 2×2, unlocked by `sports_field`, single variant) (71/73 beats — the 2 missing are the ratio-warning beats needing the §7 TriggerRule extension), `train_station` dropped (the rail can't be drawn coherently). Road-tile sprites done, non-square footprints fixed (#90), **land expansion shipped (device-verified)**, ✅-marker sync script live. **All sprites now affine-fit their footprints** — the original 16 dimension-mismatch flags are all cleared (15 by NB regeneration, `clinic` by re-footprinting 2×1); no squash placeholders remain. Phase 8 (the city_builder.md design doc) is complete. Phase 7's headless work is complete; the remaining device-verification sweep is owed — now doable on the local Android emulator (the web-sandbox no-emulator blocker no longer applies). Phase 6 left two deliberately-deferred sub-tracks: (a) selective DeepMind expansion (variety only, optional); (b) adaptive-system + mix-policy tuning (needs playtest data first). Live counts for the curriculum are in [curriculum.md](curriculum.md).
- **Last updated:** 2026-09-20
- **Last action (2026-09-20, branch `phase-10-sites-live`, PR):** **The construction loop is playable on device.** Schema **v16** (wipe-and-recreate): `ConstructionSites` table (`goalKind` building | land, building / land columns, `paidCoins`, `upgradesFromPlacementId`, `startedAtRound`, no status column), `Players.coinBalance` and `BuildingPlacements.currentTier` dropped; `openSite` turns a full site into a placement (removing an upgrade's source) or an owned block inside one transaction ([construction_sites.dart](lib/data/construction_sites.dart) maps rows to the domain value). **Routing:** `recordAnswer` pays answer + bonus into `activeSiteIdProvider`'s site via `CityActions.payIntoSite`; the answer that fills the bar opens it; `AnswerReward.sitePayIn` + `QuestionBlock.siteAfter / sitePaidBefore / siteOpened` feed the summary. **City screen:** placing a catalog building starts a site (no purchase, no affordability, a fourth is refused naming the three); sites draw at their stage (dirt pad + fence → slab → ghost sprite, procedural, no crane/scaffold sprites yet), are selectable and movable, and the site bar shows `paid / price` with *Build!* (sets the active site, pushes the wheel); frontier blocks start land sites (dirt block); the Play-math FAB and every coin balance are gone; unlock rules count ladder ancestors. Spin/question AppBars carry the site's compact bar (first cut overflowed the AppBar — a progress bar has no intrinsic width; fixed with `IntrinsicWidth` + a widget test); the block summary gets a site card and *Back to city* once the site opened. `flutter analyze` clean, **935 tests pass** (+site persistence, routing, block getters, AppBar bar). **Device-verified on the Pixel 7 AVD** from a wiped install: create player → mayor's office opens instantly → demand bubble → home site 0 / 60 → Build! → wheel shows 0 / 60 → correct answer bumps it to 2 / 60 → summary card 29 / 60 ("was 0 before this block") → next block fills it, summary reads *It's open!* 60 / 60 with *Back to city* → the house stands, population 3, praise beat fires. A land site (0 / 1200) started by accident and survived an app restart alongside the home site with coins intact; the +20 debug pay steps showed the slab at 20 / 60 and the ghost at 40 / 60. **Seen live, for the overflow question:** the opening block earned 48 but the site took 31 — 17 coins went nowhere (lifetime still counted them).
- **Prior action (2026-09-20, PR #113 rebase-merged to `main`):** **Phase 10 domain layer — construction sites, pure Dart, green.** New [construction_site.dart](lib/domain/city/construction_site.dart): sealed `SiteGoal` (`BuildingGoal` with tile anchor + optional `UpgradeLink`; `LandBlockGoal` priced by `blockCost`) — *everything with a price is a site, there is no wallet*; immutable `ConstructionSite` (goal, `paidCoins`, `startedAtRound`) with `payIn` / `payReward(AnswerReward)` returning a `PayInResult` (accepted, overflow, `opened`, `stageAdvanced`); `stageForFraction` on the draft 0 / ⅓ / ⅔ / done thresholds; `checkStartSite` enforcing the 3-open-sites cap, one open upgrade per source placement, adjacent-ladder-rung only, land-frontier adjacency and one site per block. New [upgrade_ladders.dart](lib/domain/city/upgrade_ladders.dart): the §8.6 ladders declared (housing spine; power; water; clinic→hospital; school→high_school; mayors_office→town_hall→city_hall) **plus the two authored entertainment ladders** `park → botanical_garden → zoo` and `sports_field → stadium → amusement_park`; `nextRung`, `isUpgradeStep`, `upgradeDeltaPrice` (target − source), and `placedWithLadderAncestors` so upgrading a building away never re-locks what it had unlocked. `netPopulationOnOpen` = target − source; the old building keeps counting in `populationCapacity` because sites contribute nothing until they open. **Cancel semantics resolved (§8.11): no cancel in v1** — paid coins would have nowhere to go without a wallet, transfer needs a picker plus a no-other-site edge case, and the 3-site cap already supplies the pressure; so the `ConstructionSites` table needs no status / cancelledAt column: a row lives from placement to opening and its only exits are *moved* and *opened*. 45 new tests in [test/domain/city/](test/domain/city/) incl. coverage: every registry building starts as a site and opens on full payment; every ladder is strictly price-monotonic, never shrinks its footprint, stays in one category, and every step starts as an upgrade site. `flutter analyze` clean, **915 tests pass**. Schema, reward routing and presentation untouched.
- **Prior action (2026-09-19, docs only, on `main`):** **Designed the construction loop — the answer to "the city builder and the questions feel like two disconnected apps."** A brainstorm session: 18 directions (day cycles, chapter books, mayor's desk hub, jobs-by-building, zoom-not-navigate, construction sites, contracts, delivery runs, citizen faces, a companion, a persistent goal rail, …) → the user picked *construction sites* (the question IS the building) → knob-by-knob decisions. **Decided:** placement creates a *construction site*, not a purchase; the camera zooms to it and the wheel appears above it (zoom, never swap screens); every coin earned pays the site down against a `paid / price 🪙` bar and the site's art advances through stages with no stage text; **coins stay, the wallet goes** (a coin only exists inside a site; `coinBalance` is dropped, `lifetimeCoinsEarned` stays); streak / wrong-answer / keypad rules unchanged; up to 3 open sites; block end stays zoomed in with *Spin again*; **upgrades** are sites priced at the difference whose grown footprint may relocate anywhere on owned land, the old building standing until the new one opens; v1 = construction + upgrades, festival / repairs / crew deferred; interim stage art is generic overlays (pad, fence, crane, scaffold, ghost), bespoke stage art later. **Rejected units of progress and why:** shifts/days (fixed-size referent lies under the streak multiplier), question count (breaks same-study-time-per-grade), percent (relative), stage labels (art carries it). Written into city_builder.md §8, prd.md (Gameplay Loop, Cosmetics, City Builder), and Phase 10 below; Phases 10–13 renumbered 11–14.
- **Prior action (2026-09-18, PR #112 merged to `main`):** **Spin wheel redesigned as a Math City carnival wheel.** Three rounds of HTML mocks (20 directions → 6 rims → NEW placement + animated spin effect; links in the PR) settled the spec, then it was ported to Flame: (1) `shortLabel` text on wedges replaced by **icon ladders** — one object per category, three rungs by the concept's grade band (K–2 / 3–5 / 6–8), painted in the logo's flat outlined style ([concept_icons.dart](lib/game/spin_wheel/concept_icons.dart)); icons sit radially at 74 % of the radius on white badges and rotate with the wheel, as does the curved orange **NEW!** under a never-played concept's icon (the review ✓ sticker is gone). (2) The black Flame canvas + navy warp overlay replaced by the **sky gradient and home-strip city** ([city_backdrop.dart](lib/game/spin_wheel/city_backdrop.dart)); the spin effect is a horizontal blur of the city plus pale streaks flying out from the rim, both scaled by angular velocity, and the wheel itself picks up a rotational blur (ghost copies of a cached wheel image) at speed. (3) Carnival rim (red/cream stripes, bulbs, wooden pointer, star hub); wheel centred on the sky band above the strip. (4) Landing reveals the concept's full `name` in a pill below the wheel ([reveal_pill.dart](lib/game/spin_wheel/reveal_pill.dart)); the first-landing "New topic!" card still follows. Category colours re-tinted toward the logo palette ([category_colors.dart](lib/presentation/theme/category_colors.dart)). Device-verified on the Pixel 7 AVD: at rest, mid-spin (blur + streaks, no dark edge), landed. Domain layer untouched.
- **Next action:** **Phase 10 step 5: camera zoom + wheel overlay.** Decide the overflow policy first (it changes what the summary says when a block overshoots — the default today drops the excess). Then, in order: (5) schema bump dropping `coinBalance`; (3) reward routing into the active site; (4) city-screen sites + HUD bar; (5) camera zoom + wheel overlay + summary card; (6) beat → *Build it* highlight and the first-run path; (7) upgrades; (8) generic stage art. Resolve the §8.11 open questions as they're hit (cancel semantics first — it shapes the schema). Then land as a PR and `/ux-sweep` the new site endpoint.
- **Prior next action (2026-09-11, folded into Phase 10):** **tune the coin prices by playtest** (PR #110 merged to `main` 2026-09-11) — the building table, land ladder (600 × ring), lifetime gates (1–4 h) and `expected_seconds.dart` are all first-draft numbers in one place each. Follow-ups noted while building it: (a) ~~the drip-feed still picks lowest-grade-first, so a grade-2 player's first unlock was `count_to_10` (K, auto-mastered at p = 0.95)~~ — **fixed 2026-09-16** (it was not harmless: it drained a grade-2+ player's wheel to zero; see *Wheel composition* below); (b) `AudioService` hooks for coin collection / band bonus are still stubs (prd.md lists both sounds); (c) emoji → sprite pass app-wide (Nano Banana batch) is separate. Phase 7's device-verification sweep is still owed; Phase 6's deferred sub-tracks stand.
- **Last action (2026-09-16, PR #111 merged + two follow-up commits on `main`):** **Wheel always 8, rotates every spin, mastered concepts come back as review.** Diagnosed with the real engine: the wheel was pinned at 4 because the starter pack was 4 and each mastery removed one concept and introduced exactly one (net zero), and for grade-2+ players every unlock went to a K concept that started auto-mastered (p = 0.95) → the wheel drained to 0 and the fallback pinned the 4 mastered starters forever. Spec agreed and implemented as *Wheel composition* below: new pure-domain [wheel_selection.dart](lib/domain/concepts/wheel_selection.dart) (tiers + slot rules + rotation), `DripFeedEngine.topUp` keeps 12 frontier concepts active, `pickNext` skips concepts that would start mastered, `startingProficiency` gives an introduced above-grade concept the challenging floor instead of a dead `notYet`, `AnswerReward.unlock` → `unlocks` list, `lastWheelProvider` remembers the last shown wheel. UI: "NEW" sticker on never-played segments, first-landing celebration overlay on the wheel (auto-advances in 2.4 s so the emulator flow never stalls), block summary shows a one-line "N new topics are on the wheel!" teaser with category-coloured placeholder badges (per-concept wheel icons are a later pass). `flutter analyze` clean, **862 tests pass** (+~25 new: tier/slot/rotation, top-up, auto-mastered skip, review slots, rotation against the last wheel). **Device-verified on the emulator** (fresh grade-3 player): 8 segments from the first spin, every unplayed segment stickered NEW, first-landing celebration → 4-question block → summary → "Spin again" gave a wheel with 4 of 8 segments swapped and the played concept's sticker gone. *Not* seen live: the summary's new-topics teaser (needs a mastery; the deterministic fling wouldn't land the same concept twice, so it is unit-tested only).
- **Prior action (2026-09-08..11, PR #110, merged):** **Single-currency coin economy shipped end-to-end** — the spec below implemented as specified, plus the band-crossing coin bonus (2 × expectedSeconds, once per concept per threshold, reusing `ConceptBandMilestones`; replaces the old +1 🔬). New pure-domain `lib/domain/economy/` (expected_seconds for all 366 concepts, coin_economy tunables + rules, band_crossings, question_block); schema **v14** (coin columns + `streakCount`, research + `BuildingTypesResearched` dropped, wipe-and-recreate); research flow deleted everywhere (catalog = every building whose rule passes, bought on placement); all 55 buildings, the land ladder and lifetime gates re-denominated in minutes of study (city_builder.md §3/§3.5 updated, table in the PR); question blocks (spin → N questions → coin flight into the AppBar counter → red screen mid-block → `BlockSummaryScreen` → wheel), with a bigger in-place celebration + second flight for band crossings; wheel retirement (≥2 grades below + comfortable); `CoinIcon`/`CoinAmount` CustomPainter replacing every 🧱/🔬. `DebugHarness` / `ConceptDebugScreen` keep single-question semantics so `/ux-sweep` is unchanged. `flutter analyze` clean, **841 tests pass** (+~60 new: reward/streak/block/bonus/coverage/retirement/catalog/coin-reward). **Device-verified on the emulator** — [ux_reports/2026-09-09-coins-economy/report.md](ux_reports/2026-09-09-coins-economy/report.md): spin → 4-question block → coin flight → wrong-answer red screen mid-block → summary (coins, streak, "Mastered!" bonus, unlock) → wheel; bonus burst captured live; single home bought instantly for 60 coins after reading its demand; home card / city bar / debug sheet all coins-only. `/ux-sweep 3.3` smoke on the final build proves the harness still works: [ux_reports/2026-09-09-3.3-both](ux_reports/2026-09-09-3.3-both/report.md) → 30 probed/reviewed, 16 clean, 14 could-improve, 0 bugs. The improves are all explanation-wording / diagram notes on content this branch doesn't touch (10 of the 14 were rated clean by the 2026-09-05 reviewer — stricter reviewer, not regressions), `build_report.py` exit 0. One-off Riverpod debug assertion on `activeCityProvider` during the first `/open` after a long manual session, not reproducible — filed as a GitHub issue. Retirement was verified by unit test only (a fresh profile's outgrown concepts start mastered, so they never reach the wheel to begin with).
...
- **Deferred:** Audio SFX + background music (CC0 assets not sourced yet — stub in place). iOS verification. Both revisit before Phase 13 at latest.
...
| **Construction loop** (2026-09-19) | Buildings are placed as **construction sites and paid down by answering questions while zoomed in on them**. Coins stay as the unit; there is **no wallet**. Up to 3 open sites; upgrades are sites priced at the difference and may relocate. Spec: [city_builder.md §8](city_builder.md) | The city and the question flow read as two apps joined by a coin balance and a FAB. Removing the wallet removes the abstraction between them; zooming (not swapping screens) makes the question flow feel like going closer to your city. Coins survive because kids read them instantly (Mario), and because money is a unit whose size is naturally variable — so streak/keypad/difficulty weighting stays legible, which "shifts", "days", and question-count would not |
...
### Wheel composition — revised 2026-09-16 (supersedes the Phase 5 "starter pack of 2/4, one unlock per mastery, `min(introduced, 8)` segments" rules)

**Problem it solves.** With a 4-concept starter pack and a one-in/one-out drip-feed, the wheel never grew past 4 and the same 3–4 labels showed every spin. For grade-2+ players it was worse: the lowest-grade-first pick handed out K concepts that *start* mastered, so each mastery shrank the wheel by one until the fallback pinned the mastered starters forever. Content was never the bottleneck (364/366 concepts have generators).

**Decisions (user, 2026-09-16):**
- **8 segments whenever the pool allows** (`kWheelSegments`). Everything lives in [lib/domain/concepts/wheel_selection.dart](lib/domain/concepts/wheel_selection.dart), pure Dart, unit-tested.
- **Two tiers.** *Frontier* = introduced ∩ implemented ∩ challenging-or-comfortable ∩ not retired. *Review* = introduced ∩ mastered ∩ not retired — mastered concepts are **back on the wheel**, capped at **`kReviewSlots` = 2** segments (keypad mode, normal pay; `expected_seconds` pricing and retirement bound any coin farming). If one tier can't fill its share the other does, so the wheel is only short when both are.
- **Active pool target `kActivePoolTarget` = 12.** The starter pack is 12 (same easiest-at-the-frontier policy, ignoring prereqs as before). After every *correct* answer `DripFeedEngine.topUp` introduces concepts (via the unchanged `pickNext` policy: lowest grade → category with fewest active → row order, prereqs mastered, generator registered) until the frontier count is back at 12 — so a mastery or a retirement refills the pool, usually by one. Wrong answers never unlock anything.
- **Rotation `kMinRotation` = 3.** At least 3 of the 8 segments were not on the wheel the player last saw (`lastWheelProvider`, session-only), pool permitting. Frontier absorbs the quota first, review takes the remainder.
- **Two drip-feed fixes that fell out.** (1) `pickNext` never picks a concept whose starting proficiency is already mastered (they satisfy prereqs without being introduced). (2) `startingProficiency(introduced: true)` floors an *introduced* above-grade concept at 0.40 (challenging) instead of 0.05 (`notYet`): the drip-feed only introduces it once its prereqs are mastered, and at 0.05 nothing could ever lift it onto the wheel — a K player who mastered all of K would have had grade 1 introduced invisibly.
- **Unlock UX moves to the wheel.** Segments the player has never answered (no proficiency row) carry a **"NEW" sticker**; the **first landing** on one shows a celebration overlay before the block (tap or 2.4 s auto-advance). The block summary keeps a **one-line teaser** ("N new topics are on the wheel!") with category-coloured placeholder badges — the per-concept icons the wheel will eventually carry replace those badges later.
- **Mastery speed left for playtest.** α = 0.1 and the 0.85 threshold are unchanged (14 straight correct from 0.40). A wider pool visits each concept less often, so masteries and band bonuses come slower; fold retuning into the coin-price playtest.

**Follow-ups (done 2026-09-16, direct to `main`):** (1) **Grades interleave.** The starter pack is drawn round-robin across in-band grades, at-grade first (a grade-3 player starts 6 × grade-3 + 6 × grade-2), and `pickNext`'s step 1 is now "stay at-or-below the player's grade while anything there is eligible; among those grades prefer the one with the fewest active concepts, tiebreak higher" — above-grade content only once at-or-below is exhausted, lowest first. (2) **Review sticker.** Mastered concepts on the wheel carry a small teal ✓ pill (same sticker as NEW, different colour). Unit-tested (6/6 split, grade-balanced refill, grade-6 player never gets grade-7 while ≤6 remains); the sticker is not device-verified (needs a mastered concept on a fresh profile).
...
- [x] **Exit criteria (Path B — Android only):** `flutter run` launched the placeholder Math City app on the Android emulator (Pixel 7, API 34) successfully. iOS verification deferred to Phase 12
- [ ] iOS exit criteria — deferred until Xcode is installed (no later than Phase 12)
...
- [ ] Basic SFX (spin, correct, wrong) and one looping background track — deferred to Phase 12 (CC0 assets not sourced; stub in place)
...
- **DAG drip-feed semantics.** *(Superseded 2026-09-16 — see Domain Specs → Wheel composition: starter pack of 12, top-up to 12 after every correct answer, auto-mastered concepts never picked.)* New player starts with 2 implemented concepts (lowest grade, lowest within-category row order). One new concept is introduced per mastery event. A concept is eligible for introduction only when all its DAG prereqs are `mastered` *and* its generator is registered. **No cross-domain gating** — independently per branch. **Pick policy:**
...
- **Unlock UI moment.** *(Superseded 2026-09-16 — the wheel carries "NEW" stickers and a first-landing celebration; the summary keeps a one-line teaser.)* A "New concept unlocked!" celebration card is shown after the result screen, but only on correct answers that triggered either a mastery event or a queue advance. Never on wrong answers — this avoids the confusing scenario where a kid gets a question wrong but sees a celebratory unlock.
...
- **Drift v4: wipe-and-recreate.** No real users yet, so the existing v3 migration pattern is fine. A proper additive-migration implementation is now a prerequisite task in Phase 13 (Cloud Save) — once player data lives off-device, wipes destroy real data.
- **Wheel segment count.** *(Superseded 2026-09-16 — always 8 when the pool allows, 2 of them review slots; see Domain Specs → Wheel composition.)* Fixed cap at 8. The wheel renders `min(introducedConceptCount, 8)` segments and never more.
...
- ~~Building upgrade tiers — up to 3 visual tiers per building; upgrading costs currency; footprint stays the same~~ **Superseded 2026-09-19** by Phase 10 upgrades (distinct types, delta-priced construction sites, footprint grows and may relocate — city_builder.md §8.6)
- ~~Currency-funded events: 3–5 event types (festival, marketing campaign, etc.) — temporary growth boost in exchange for spend~~ **Deferred past Phase 10** (a festival becomes a community construction site — city_builder.md §8.10)
...
### Phase 10 — Construction Loop: one app, not two (target: ~4–6 weeks)

**Goal:** Make answering questions *the act of building*. The city screen and the question flow currently feel like two apps joined by a coin balance and a FAB. Spec: [city_builder.md §8](city_builder.md) (designed 2026-09-19 — brainstorm of 18 directions narrowed to construction sites paid down on site, no wallet, zoom-not-swap camera, delta-priced relocatable upgrades). Multi-layer + schema migration → build on a branch, land as a **PR**.

- [x] **Domain (pure Dart, tested first — done 2026-09-20):** `ConstructionSite` value type + rules — pay-in from an `AnswerReward`, paid fraction → stage index, open-on-full, the 3-open-sites cap, upgrade delta pricing (`target.coinCost − source.coinCost`), `upgradesFrom` link, net-population on open. Land blocks and parks are sites too. ([construction_site.dart](lib/domain/city/construction_site.dart), [upgrade_ladders.dart](lib/domain/city/upgrade_ladders.dart); ladders confirmed per §8.6 and the entertainment ladders authored there.)
- [x] **Schema bump (wipe-and-recreate, pre-launch precedent — v16, 2026-09-20):** `ConstructionSites` table (cityId, goalKind, buildingTypeId?, gridX/Y?, blockX/Y?, paidCoins, upgradesFromPlacementId?, startedAtRound — **no status column**: v1 has no cancel, a row's only exits are moved / opened); **drop `Players.coinBalance`** (`lifetimeCoinsEarned` and `streakCount` stay); retire the in-place tier columns.
- [x] **Reward routing (2026-09-20):** coins from a block go to the active site, never to a balance. `QuestionBlock` / the proficiency notifier learn which site is active; block summary reads the site's before/after.
- [x] **City screen — sites (2026-09-20; zoom deferred to the camera item):** placing a catalog building creates a site (no purchase, no affordability check); site HUD bar `paid / price 🪙` with *Build!*; ~~tap a site to zoom into it~~ (tap selects it, *Build!* opens the wheel — the zoom lands with the camera item); a fourth site is refused with a nudge naming the three; sites are movable like buildings, not cancellable (v1). Land blocks start land sites.
- [ ] **Camera + wheel overlay:** Flame camera tween to the site, site anchored to the bottom of the viewport, dim layer, `SpinScreen` → an overlay state of `CityScreen` (PR #112 wheel art unchanged; its static backdrop replaced by the live camera). Back from site-zoomed = zoom out, not pop. `ResultScreen` → summary card over the site with *Spin again* / *Back to city*; auto zoom-out when the building opens. Question screens stay full-screen routes (confirm).
- [ ] **Commit screen:** price + personalised estimate ("about N play sessions at your pace") from recent coins-per-block; shown once before placing, never in the HUD.
- [ ] **Beat → build:** demand beat highlights the building(s) it unlocks in the catalog and its expanded card carries *Build it*; first-run path (mayor's office → `demand_first_home` auto-expanded → place → zoom → wheel) needs no menu. Praise beat on open reads as the mayor replying to the citizen.
- [ ] **Upgrades:** a placed building's catalog shows its next rung(s) at the delta price; the upgrade footprint may be placed anywhere on owned land (over the old building if it fits); the old building stands and counts until the upgrade opens, then its plot clears. Confirm which arcs are ladders per city_builder.md §8.6; author the entertainment ladder.
- [ ] **Stage art (interim, generic overlays — city_builder.md §8.8):** ~~procedural dirt pad + fence hoarding parameterised by footprint~~ (done 2026-09-20, plus a stage-1 slab and the stage-2 ~35 % ghost, all in `CityBoardComponent`); crane + scaffold sprites through the §5.1 pipeline still to do. Bespoke per-building stage art for hero landmarks is a later batch.
- [ ] **Debug harness:** keep every `DebugHarness.instance.attach*` call intact through the overlay refactor; add a site endpoint so `/ux-sweep` can put a zoomed site on screen.
- [ ] **Exit criteria:** A brand-new player goes from mayor's office to an opened first home without touching a menu. A city with 3 open sites survives an app restart with every paid-in coin intact. A single home upgrades to an apartment on a relocated plot, the old home standing until the apartment opens. No coin balance is visible anywhere in the app. The `/ux-sweep` still runs green.

---

### Phase 11 — Player Progress Screen (target: ~1 week)
...
### Phase 12 — Sound, Polish, Engagement (target: ~2–3 weeks)
...
### Phase 13 — Cloud Save (target: ~1–2 weeks)
...
### Phase 14 — Beta + Store Submission (ongoing)
...
- **Phase 10 (found while writing the domain layer, 2026-09-20):**
  - **Upgrades re-locking unlock rules.** `requiredBuildingsPlaced` is evaluated live, so opening `town_hall` (which removes the mayor's office) would re-lock `single_home`. Domain fix is ready — feed `placedWithLadderAncestors(placedTypeIds)` to `UnlockContext` — wire it in the catalog provider during step 3/7.
  - **`luxury_condo` (50) houses fewer than `high_rise` (60)**, so the top housing upgrade has a *negative* `netPopulationOnOpen`. Fold into the Phase 10 price / population tuning pass (bump the condo or accept "fewer, fancier").
  - **Overflow coins.** A reward that overshoots a site's price comes back as `PayInResult.overflow`; `lifetimeCoinsEarned` already counts the whole reward. **Routing today drops the excess** (seen live: a block earned 48, the home took 31, 17 vanished — and the rest of the block after the open pays no site at all). Decide: end the block on open, roll the excess into the oldest other open site, or accept the loss.
...
- **By Phase 10 (resolved 2026-09-19):** **How the city and the questions connect** — construction sites paid down on site; coins stay as the unit, the wallet goes; zoom-not-swap camera; up to 3 open sites; delta-priced upgrades that may relocate. See [city_builder.md §8](city_builder.md).
- **By Phase 10:** the §8.11 list — cancel-a-site semantics (no wallet means paid-in coins have nowhere to go), question screens full-screen vs over the zoomed city, stage thresholds, landmark price cuts on top of delta pricing, which arcs are upgrade ladders, the commit-screen estimate formula, land blocks as sites.
- **By Phase 12:** Music + SFX sourcing (CC0 from Freesound.org and OpenGameArt.org).
- **By Phase 13:** Are we OK requiring the user to sign in to Game Center / Play Games for cloud save? Otherwise local-only is the only option.
- **By Phase 14:** Privacy policy text — minimal since we collect ~nothing, but COPPA considerations for under-13 audience need legal review or a template.
...
| Cloud save platform divergence | Low (single package) | Medium | `games_services` abstracts both — but test on both platforms early in Phase 13 |
...
| COPPA / children's-app compliance | Medium | High (could block store submission) | No data collection; address this explicitly in Phase 13 with a minimal privacy policy and store-listing kids-category settings |
