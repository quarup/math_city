...
- **Phase:** Phase 6 — Full Question Bank. Catalog parser landed; ~340 catalog-only concepts now flow through the wheel filter. Next: word-problem framework or priority-list generator family (your pick).
- **Last updated:** 2026-05-10
- **Last action:** Built `tools/curriculum/build_catalog.dart` (parser script) and regenerated `lib/domain/concepts/concept_registry.dart` from [curriculum.md](curriculum.md) §3.x tables. Highlights: (a) Parser walks each `### 3.x ... (`category_id`)` heading + table, emits one `Concept(...)` row per data row; row order within a category is dense 0..N-1 and matches existing hand-tuned indices exactly. (b) Two override maps in the parser: `_shortLabelOverrides` (kid-facing wheel labels for the 22 concepts that already had them — defaults to the curriculum.md Display column otherwise) and `_prereqOverrides` (12 transitional simplifications that drop prereqs whose generators don't exist yet, so the drip-feed engine can still reach the implemented concepts; each entry is keyed for one-line removal as new generators land). (c) Catalog grew from 22 hand-authored rows to **362 rows across all 12 categories** — covers the entire K–8 taxonomy from `count_to_10` through `pythagorean_distance_coords`, `volume_sphere`, etc. (d) Parser includes its own grade-DAG validator that mirrors the catalog-invariant test, so curriculum.md authoring errors fail at build time with a clear message. (e) Surfaced and fixed two pre-existing curriculum.md authoring inconsistencies via the new validator: `simplify_fraction` (G4) listed `gcf_two_numbers` (G6) as a prereq, and `add_fractions_unlike_denom` (G5) listed `lcm_two_numbers` (G6) — both fixed by dropping the formal-only prereq, since informal simplification/common-denominator is reachable before grade 6. (f) Drive-by fix: `test/widget_test.dart` had been broken since the splash screen was added (looked for `find.text('Math City')` which is no longer rendered as a Text widget); updated to pump past the splash and look for the empty-state Create Player button. (g) **All 105 tests pass** (up from Phase-5's 91 — gain is the dag_engine "real catalog" tests that now exercise the full 362-row catalog plus the unrotted widget test); `flutter analyze` clean. The `concept_registry.dart` file is now machine-emitted (~4k lines) and carries an `ignore_for_file: lines_longer_than_80_chars` for the few long prereq lists.
- **Next action (your pick):** Two natural sub-slices of Phase 6, generators-first ordering:
  - **(A) Word-problem framework** ([curriculum.md §5.1 #5](curriculum.md)) — `lib/domain/questions/word_problems/`: name pool (~25 culturally diverse names), item pool (~20 countables), verb pool (~10 contexts), 1-step template engine. First two generators target `add_word_problems_within_100` and `add_sub_2step_word_problems`. Worth getting your input on the pool contents (cultural diversity choices, vocabulary level) before I author them.
  - **(B) Multi-digit × ÷ generator family** ([curriculum.md §5.1 #2](curriculum.md)) — adds `mult_4digit_by_1digit`, `mult_2digit_by_2digit`, `mult_multidigit_standard_alg`, `div_4digit_by_1digit`, `div_4digit_by_2digit`, `div_with_remainder` (+ unblocks `interpret_remainder_word`, `mult_compare_word`, `mult_div_word_2step` once word-problem framework lands). No new diagrams needed; pure algorithmic. Lower-design-input; I can land this autonomously.
- **Pending review:** Working-tree diff (uncommitted): the parser, the regenerated registry, two curriculum.md fixes, the widget_test fix, plan.md updates. Suggest committing as a single "Phase 6 catalog parser" change once you've eyeballed.
...
*Generators-first sub-track (priority list #6 onward, with diagram widgets interleaved as each generator family needs them)*
