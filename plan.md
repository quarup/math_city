...
- **Phase:** Phase 6 — Full Question Bank. **130 generators implemented**; broad K–8 coverage. Remaining gaps: scientific notation, 3D geometry, coordinate plane, transformations, Pythagorean, data displays (bar/box/histogram), compound probability.
- **Last updated:** 2026-05-17
- **Last action:** **Chunk 16 — number systems + exponents** — 5 generators appended to `number_theory_generators.dart`: `prime_or_composite` (G4, MC over {prime, composite, neither, both}), `exponents_whole_number` (G6, "2^3 = ?" → 8; exponent capped per base so values stay ≤ ~256; override drops the unimplemented `powers_of_10`), `order_of_operations_with_exp` (G6, three shapes: a+b², a×b², a−b²; exponent-first precedence is the lesson), `sqrt_perfect_squares` (G8, "√81 = 9"; root ∈ [2, 15]), `cbrt_perfect_cubes` (G8, "∛27 = 3"; root ∈ [2, 8]). Override cleanup: removed `evaluate_expression: ['order_of_operations_no_exp']` since `order_of_operations_with_exp` now lives. Tests: +5 groups @ 300; **309 total pass**, `flutter analyze` clean.
- **Last action (prior):** **Chunk 15 — geometry + probability cleanup** — 5 generators. Commit `8403374`.
