...
- **Last action:** **Demand-beat gating — a building's research card only appears after the player opens the beat that asks for it.** Previously, placing the mayor's office + earning 🔬 surfaced *all* nine non-starter buildings as locked cards at once. Now each building's `unlockRule` carries `requiredBeatsRead` naming its demand beat, and the card stays hidden until that bubble is *opened*. **Mechanics:** renamed the (unused-by-buildings) `UnlockRule.requiredBeatsFired` → `requiredBeatsRead` and `UnlockContext.firedBeatIds` → `readBeatIds`; new `db.readBeatIds` (beats with non-null `ackedAtRound`) feeds `cityCatalogProvider`; `markBeatRead` now also invalidates the catalog. **Read = open:** the bubble sticker's tap-to-expand now calls `markBeatRead` (so opening unlocks the building and starts the linger timer); "Got it" just closes the card. **Content:** authored 4 new demand beats so every building has an ask — `demand_school`, `demand_apartment` (pop≥8), `demand_grocery`, `demand_coffee_shop` (registry now 13 beats); mapped beats→buildings (first_home→single_home, school→school, apartment→apartment, clinic→clinic, power→power_plant, waste→waste_management, grocery→grocery, coffee→coffee_shop, more_parks→park). **Docs:** prd.md unlock model + bubble section, plan.md schema/specs. **Tests:** rewrote `dag_engine_test` + `city_catalog_test` for the read-gate, renamed fields in `unlock_rule_test`/`building_registry_test`. All 713 tests pass; analyze clean.
- **Prior action:** **Beat pacing — space out new bubbles + keep read bubbles up a few rounds.** Two tweaks to the citizen-bubble flow so beats don't all dump at once and don't blink out on tap. **(1) Trickle:** `CityActions.fireBeats` now fires at most **one** new beat per pass, and only when ≥`kNewBeatSpacingRounds` (5) rounds have elapsed since *any* beat last fired (first-ever fire always allowed) — so a single build that makes several beats eligible (e.g. first home → praise + clinic + power demands) releases them one every ~5 rounds instead of bursting. **(2) Read-linger:** reading a bubble no longer dismisses it instantly. New `StoryBeatStates.ackedAtRound` column (Drift **v9**, wipe-and-recreate) stamps the round the player read it; the sticker stays `onScreen` for `kReadHideRounds` (3) more rounds of math play, then `fireBeats` retires it to `acked` (re-fireable). `onScreenBeatsProvider` honours both windows (un-read → rotate at 8; read → hide at 3). `dismissBeat` → `markBeatRead` (DB `setBeatState` → new `markBeatRead`); `recordBeatFired` clears `ackedAtRound` on re-fire; the "Got it" button now marks-read-and-lingers. **Tests:** rewrote `beat_firing_test` for the trickle (beats now drained round-by-round via a `_drainUntil` helper) + a new spacing assertion + read-linger/retire test. All 710 tests pass; analyze clean.
