...
- **Phase:** Phase 6 — Full Question Bank. **179 generators implemented**; broad K–8 coverage. Remaining gaps: coordinate plane, transformations, data displays (bar/box/histogram), compound probability, some 3D volume (cylinder/cone/sphere need π), most K–2 counting/place-value foundations, angle concepts (need `angle_diagram` widget).
- **Last updated:** 2026-05-17
- **Last action:** **Chunk 25 — G8 algebra + triangle inequality + patterns** — 5 generators spread across three existing files. Equations: `solve_linear_eq_no_or_inf` (G8, three case shapes: one solution / no solution / infinitely many — kid picks the case, not the value; CCSS 8.EE.C.7.a), `solve_linear_eq_with_distrib_collect` (G8, "Solve p(x+q) − kx = r" — one step beyond `solve_two_step_eq_distributive`). Geometry: `triangle_inequality_recognize` (G7, MC over {yes, no, only if all equal, can't tell} — no diagram needed since the inequality is purely a sum comparison). Patterns: `numerical_pattern_rule` (G4, both arithmetic and geometric continuation), `arithmetic_patterns_in_tables` (G3, multiples-of-n continuation testing CCSS 3.OA.D.9). New overrides: `numerical_pattern_rule` drops `skip_count_2` (no gen) → `mult_facts_within_100`; `triangle_inequality_recognize` drops `shape_attributes_basic` (no gen) → `compare_2digit`. Tests: +5 groups @ 300; **359 total pass**, `flutter analyze` clean.
- **Last action (prior):** **Chunk 24 — unknown-position + signed quantities + theoretical vs experimental** — 5 generators. Commit `c9b7575`.
