...
- **Last action:** **Chunk 5 (partial) — population growth model v1 (pure-Dart, fully unit-tested; not yet wired into the city).** New `lib/domain/city/population_model.dart` with two pure functions. `populationCapacity(Iterable<BuildingType>)`: base = `sum(populationContribution)` capped by the thinnest *gating* service (`{power, clinic, waste}`, each granting a `serviceFreeAllowance` of 20 so a hamlet survives with no infrastructure — `school` is a soft service and does not gate), then scaled by a desirability multiplier (`+0.05` per distinct variety type, capped at `1.5`, ×`0.8` lopsidedness penalty when commercial+entertainment buildings outnumber housing >2:1). `stepPopulation(current, capacity, {rate=0.25})`: closes a rounded-up fraction of the gap each tick — growing under / shrinking over / holding at capacity, landing exactly without overshoot. All numbers are Phase-7 placeholders (tuned in Phase 8/9). **Tests:** new `test/domain/city/population_model_test` — 20 cases covering housing base, the service-ratio stall + lift, school-not-gating, variety bonus + cap, lopsidedness penalty, and step growth/decline/convergence. All 658 tests pass; analyze clean.
- **Prior action:** **Chunk 3 — research folded into the build catalog (no separate panel; design call 2026-05-24).** Rather than a standalone research screen, the bottom build-mode catalog now lists *both* researched buildings (placeable; 🧱 cost) and buildings currently *available to research* (locked; 🔬 cost + corner lock badge) in one scrolling row, so "research a building" and "place a building" share one surface. **Data:** `AppDatabase.researchBuilding` (spend 🔬 via `incrementPlayerResearch(-cost)` → insert `BuildingTypesResearched` row; idempotent on the composite PK so it never double-charges). **State:** `researchedBuildingsProvider` replaced by `cityCatalogProvider` → `List<CatalogEntry>` (`building` + `researched` flag), unioning `researchedBuildingTypeIds` with `BuildingDagEngine.availableToResearch` (its `UnlockContext` built from lifetime-🧱 + placed-building-type IDs; population/fired-beats stubbed at 0/∅ until Chunks 5–6), emitted in registry order so a card flips locked→placeable in place; new `CityActions.researchBuilding` spends then invalidates catalog+player+all-players. **UI:** `_CatalogCard` renders a locked entry with the 🔬 cost and an `Icons.lock` corner badge — visually distinct from the existing 0.4-opacity "can't afford 🧱" dim; tapping a locked card opens a confirm dialog ("Research X? Spend 🔬N…") before spending, then auto-selects the freshly-unlocked type for placement. Researched-but-unaffordable cards stay greyed/unselectable as before. **Tests:** `researchBuilding` group in `test/data/city_placement_test` (spend + lifetime-monotone + idempotency) and new `test/state/city_catalog_test` (fresh player = mayor only; placing the mayor surfaces the locked rest; a researched type flips to placeable). All 638 tests pass; analyze clean (modulo the pre-existing test-file lint).
- **Next action:** **Wire the population model into the city (next headless-testable step).** The pure model exists; now persist + drive it: add a DB `setCityPopulation` (the `Cities.population` column already exists), recompute `populationCapacity` from the city's placements and apply one `stepPopulation` tick on a natural trigger (each placement, and ideally each answered question so playing math visibly grows the city), persist the new value, and feed it into `cityCatalogProvider`'s currently-stubbed `population` — unblocking population-gated unlocks (e.g. a future cinema). Unit-testable via `ProviderContainer` + in-memory DB (mirrors `proficiency_research_award_test`). **Then the device-verification-heavy remainder:** **Chunk 4** move-mode UI (drag/retile an existing placement via `moveBuildingPlacement`, occupied-tile rejection), **Chunk 5** auto-road generation + a visible population counter, **Chunk 6** beat-bubble overlay + the remaining ~5–10 beats (feeds the stubbed `firedBeatIds`). Phase 6 deferred work (selective DeepMind expansion, mix-policy + adaptive-system tuning, kid validation) revisited post-Phase 7. **Manual verification still owed (sandbox is headless — can't drive the Flame canvas):** pan/zoom feel, tap-to-place hit accuracy across the iso grid, that `TapCallbacks` taps aren't swallowed by `ScaleCallbacks` (fallback: detect a tap inside `onScaleEnd` when the focal point barely moved), and that the locked-card + research confirm-dialog flow reads clearly at kid scale.
...
- [x] **Population growth model v1** (pure-Dart, well unit-tested) — `lib/domain/city/population_model.dart`, 2026-05-24. Grows toward `sum(populationContribution)` modulated by:
