...
- **Last action:** **Beat pacing — space out new bubbles + keep read bubbles up a few rounds.** Two tweaks to the citizen-bubble flow so beats don't all dump at once and don't blink out on tap. **(1) Trickle:** `CityActions.fireBeats` now fires at most **one** new beat per pass, and only when ≥`kNewBeatSpacingRounds` (5) rounds have elapsed since *any* beat last fired (first-ever fire always allowed) — so a single build that makes several beats eligible (e.g. first home → praise + clinic + power demands) releases them one every ~5 rounds instead of bursting. **(2) Read-linger:** reading a bubble no longer dismisses it instantly. New `StoryBeatStates.ackedAtRound` column (Drift **v9**, wipe-and-recreate) stamps the round the player read it; the sticker stays `onScreen` for `kReadHideRounds` (3) more rounds of math play, then `fireBeats` retires it to `acked` (re-fireable). `onScreenBeatsProvider` honours both windows (un-read → rotate at 8; read → hide at 3). `dismissBeat` → `markBeatRead` (DB `setBeatState` → new `markBeatRead`); `recordBeatFired` clears `ackedAtRound` on re-fire; the "Got it" button now marks-read-and-lingers. **Tests:** rewrote `beat_firing_test` for the trickle (beats now drained round-by-round via a `_drainUntil` helper) + a new spacing assertion + read-linger/retire test. All 710 tests pass; analyze clean.
- **Prior action:** **Round clock — persistent per-player round counter, now driving building age + bubble rotation.** New `Players.roundsPlayed` column (Drift **v8**, wipe-and-recreate) — a monotonic count of answered questions that survives restarts (the "round" clock the city builder keys off). `ProficiencyNotifier.recordAnswer` bumps it once per answered question, *before* the population tick + beat eval so both see the fresh value (new DB helper `incrementRoundsPlayed`, returns the new value). `placeBuilding` now stamps `placedAtRound` with the player's live clock (was a placement-count stand-in), and `CityActions.fireBeats` builds the real `maxBuildingAgeByTypeId` map (each type's *oldest* instance = clock − min `placedAtRound`), so the age-gated `praise_established_town` milestone (mayor's office ≥10 rounds old + prior praise) now fires. `recordBeatFired` stamps `lastFiredAtRound`; `onScreenBeatsProvider` rotates an un-acknowledged bubble off screen once it's `kBubbleRotationRounds` (8) old, leaving its DB state `onScreen` so it doesn't re-fire and re-clutter. **Tests:** rewrote the `placedAtRound` test for clock semantics + 3 new in `test/state/beat_firing_test` (`incrementRoundsPlayed` monotonic/persist; age-gated beat holds at 5 rounds then fires at 10; bubble rotates off after the window). All 708 tests pass; analyze clean.
