...
- **Phase:** Phase 6 — Full Question Bank. **155 generators implemented**; broad K–8 coverage. Remaining gaps: coordinate plane, transformations, data displays (bar/box/histogram), compound probability, some 3D volume (cylinder/cone/sphere need π), most K–2 counting/place-value foundations.
- **Last updated:** 2026-05-17
- **Last action:** **Chunk 21 — associative properties + missing-factor cleanup** — 5 generators across two existing files plus one prereq-override cleanup. Add/sub: `add_3_addends_within_20` (G1, "$a + $b + $c = ?" with sum ≤ 20), `associative_add` (G1, "If (3 + 4) + 5 = 12, what is 3 + (4 + 5)?" — same prompt shape as `commutative_add` for kid recognisability; constraint `a ≠ c` keeps the re-grouping visible). Mult/div: `associative_mult` (G3, factors ∈ [2, 5] so product stays ≤ 100), `missing_factor` (G3, "? × 6 = 42" or "6 × ? = 42" → 7 — surfaces the misconception "gave the product" via `integerDistractorsWith`), `div_as_unknown_factor` (G3, "If 6 × ? = 24, what is 24 ÷ 6?"). Override cleanup: `solve_one_step_eq_mult` override tightened from `['mult_facts_within_100']` → `['missing_factor']` (the canonical curriculum prereq) now that `missing_factor` lives; `write_expression_from_words` remains dropped. Tests: +5 groups @ 200; **334 total pass**, `flutter analyze` clean.
