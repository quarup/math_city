...
- **Phase:** Phase 6 — Full Question Bank. **85 generators implemented**; three K–8 categories with broad coverage (`fractions` 26, `decimals_percent` 24, `ratios` 5 of 16). Word problems + rationals also covered. Geometry, prealgebra, and statistics still untouched. Ratios chunk 7 (cleanup: convert_units, constant_of_proportionality, etc.) and equations chunk are next.
- **Last updated:** 2026-05-17
- **Last action:** **#8 Ratios (chunk 6)** — new file `ratio_generators.dart` with 5 generators: `ratio_intro` (G6, verbal context "Mia has 3 apples and 5 oranges. ratio?" — implemented without the curriculum.md-suggested `tape_diagram` widget, which can be added later; the diagramRequirement is presentational metadata, not a correctness gate), `ratio_language` (G6, "Which is the same as 3:5?" → "3/5"; teaches a:b ↔ a/b ↔ "a to b" notation equivalence; AnswerFormat.fraction + exactString since the lesson is the surface form), `equivalent_ratios` (G6, "Complete: 2:3 = ?:12" with random blank position), `unit_rate` (G6, "60 miles in 3 hours. mph?"; 6 context tuples for verbal variety), `constant_speed` (G6, 3-way distance/rate/time: pick which is the blank). Re-rolls on degenerate a==b ratios. Tests: +5 groups @ 300; **264 total pass**, `flutter analyze` clean.
- **Last action (prior):** **#7 Decimals chunk 5** — `convert_fraction_decimal_percent` + `decimals_fluent_4ops`. Commit `ba4afef`.
...
- **Next action:** Pick the next sub-slice. Easy options remaining ([curriculum.md §5.1](curriculum.md)):
  - **#8 Ratios chunk 7 (cleanup)** — `unit_pricing`, `convert_units_using_ratio`, `unit_rate_with_fractions`, `constant_of_proportionality`, `proportional_equation`. All algorithmic, no new widgets. Leaves `ratio_table`, `double_number_line`, `ratio_to_coordinate_pairs`, `proportional_relationship`, `multistep_ratio_word`, `scale_drawing` for later (need new widgets / are dataset-tagged).
  - **#12 One-/two-step equations** — `solve_one_step_eq_addition`, `solve_one_step_eq_mult`, `solve_two_step_eq`, `evaluate_expression`, `order_of_operations_no_exp`. Pure algorithmic, no diagrams.
