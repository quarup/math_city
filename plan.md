...
- [ ] **Placement validation — road-access invariant** (pure-Dart, well unit-tested). Free placement stays (any unoccupied tile, buildings may touch); the one constraint is that **no building may be boxed in.** A placement or move is legal only if every building keeps ≥1 free orthogonal tile somewhere on its footprint perimeter. Checked **two-way**: (a) the building being placed/moved must have an open perimeter side, *and* (b) it must not seal the last open side of any neighbor it now abuts. Generalize over `BuildingType.footprint` — all Phase 7 buildings are 1×1 (literal four sides), but the check must walk the actual footprint perimeter so a future 2×2 isn't silently mis-validated. Rejection is a gentle UI nudge, not a hard error. Supersedes the bare occupied-tile rejection from Chunk 4; with this guarantee auto-road generation never has to route to a boxed-in building
