...
- **Phase:** Phase 6 — Full Question Bank. **63 generators implemented**; two K–8 categories largely covered (`fractions` 26 + first chunk of `decimals_percent` 7) plus solid word-problem coverage and Grade-7 rationals. Remaining decimals/percent (round/divide/conversion/percent), ratios (#8), geometry, prealgebra, and statistics still untouched.
- **Last updated:** 2026-05-17
- **Last action:** **#7 Decimals sub-slice (chunk 1)** — 7 new generators in a new file `lib/domain/questions/generators/decimal_generators.dart`:
...
- **Next action:** Pick the next sub-slice. Easy options remaining ([curriculum.md §5.1](curriculum.md)):
  - **#7 Decimals — chunk 2** — `div_decimal_by_whole`, `div_by_decimal`, `round_decimals`, `decimal_to_thousandths_read`, `compare_decimals_thousandths`, `decimal_to_fraction`, `fraction_to_decimal`, `percent_intro` (needs `AreaGrid` diagram wiring), `percent_of_quantity`. Finishes the percent intro and decimal-fraction conversions.
