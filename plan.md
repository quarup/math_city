...
- **Next action:** **Phase 10 step 2 on the same branch: the schema bump.** `ConstructionSites` table — `cityId`, `goalKind` (building | land), `buildingTypeId?`, `gridX/Y?`, `blockX/Y?`, `paidCoins`, `upgradesFromPlacementId?`, `startedAtRound`; **no status column** (no-cancel decision above); drop `Players.coinBalance`; retire `BuildingPlacements.currentTier`. Then, in order: (3) schema bump dropping `coinBalance`; (3) reward routing into the active site; (4) city-screen sites + HUD bar; (5) camera zoom + wheel overlay + summary card; (6) beat → *Build it* highlight and the first-run path; (7) upgrades; (8) generic stage art. Resolve the §8.11 open questions as they're hit (cancel semantics first — it shapes the schema). Then land as a PR and `/ux-sweep` the new site endpoint.
...
- [ ] **Schema bump (wipe-and-recreate, pre-launch precedent):** `ConstructionSites` table (cityId, goalKind, buildingTypeId?, gridX/Y?, blockX/Y?, paidCoins, upgradesFromPlacementId?, startedAtRound — **no status column**: v1 has no cancel, a row's only exits are moved / opened); **drop `Players.coinBalance`** (`lifetimeCoinsEarned` and `streakCount` stay); retire the in-place tier columns.
- [ ] **Reward routing:** coins from a block go to the active site, never to a balance. `QuestionBlock` / the proficiency notifier learn which site is active; block summary reads the site's before/after.
- [ ] **City screen — sites:** placing a catalog building creates a site (no purchase, no affordability check); site HUD bar `paid / price 🪙`; tap a site to zoom into it; a fourth site is refused with a nudge naming the three; sites are movable like buildings, not cancellable (v1).
...
- [ ] **Stage art (interim, generic overlays — city_builder.md §8.8):** procedural dirt pad + fence hoarding parameterised by footprint; crane + scaffold sprites through the §5.1 pipeline; stage 2 shows the final sprite as a ~35 % ghost; stage 3 is the final sprite. Bespoke per-building stage art for hero landmarks is a later batch.
...
- **Phase 10 (found while writing the domain layer, 2026-09-20):**
  ...
  - **Overflow coins.** A reward that overshoots a site's price comes back as `PayInResult.overflow`; `lifetimeCoinsEarned` already counts the whole reward. Decide in reward routing whether the block ends there, the excess is dropped, or it rolls into the next site the player zooms to.
