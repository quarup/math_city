...
- **Next action:** Pick the next sub-slice. What's still missing ([curriculum.md §5.1](curriculum.md)):
  - **Coordinate plane (next steps)** — basic plot/read landed in Chunk 25. Still missing: `coord_distance_same_line` (G6 — text-only OK since the math is just `|x₂ − x₁|` or `|y₂ − y₁|`), `polygon_on_coordinate_plane` (G6 — needs polygon support in the widget), `ratio_to_coordinate_pairs` / `dependent_independent_vars` (G6 — needs the widget for plotting). The `slope_from_two_points` and `linear_function_construct` generators landed text-only in Chunk 21 and could be upgraded to render via `CoordinatePlane` for a richer UX, but that's polish, not coverage.
  - **Transformations (G8)** — `translate`, `reflect`, `rotate`, `dilate`. `CoordinatePlane` now exists; would still want polygon-rendering support in the widget (multi-point + edges + optional second polygon to show the image).
