...
- **Phase:** Phase 6 — Full Question Bank. **73 generators implemented**; two K–8 categories largely covered (`fractions` 26 + `decimals_percent` 17 of 26) plus solid word-problem coverage and Grade-7 rationals. Remaining decimals/percent rows (`percent_change`, `simple_interest`, `commission`, `convert_fraction_decimal_percent`, `decimals_fluent_4ops`, repeating-decimal rows), ratios (#8), geometry, prealgebra, and statistics still untouched.
- **Last updated:** 2026-05-17
- **Last action:** **#7 Decimals sub-slice (chunk 3) — percent intro** — 3 new generators in a new file `percent_generators.dart` plus a new diagram widget:
...
- **Next action:** Pick the next sub-slice. Easy options remaining ([curriculum.md §5.1](curriculum.md)):
  - **#7 Decimals — chunk 4 (finance percents)** — `percent_change`, `simple_interest`, `commission`, `convert_fraction_decimal_percent`. All algorithmic, no new widgets. Could also bring in `decimals_fluent_4ops` (composite; all prereqs now exist) and `markup_markdown`/`sales_tax_tip` if dataset items aren't needed (curriculum.md tags those as `dataset` but they can be algorithmic too — same pattern as `mult_compare_word` decision).
