...
- **Phase:** Phase 6 — Full Question Bank. **100 generators implemented**; four K–8 categories with broad coverage (`fractions` 26, `decimals_percent` 24, `ratios` 10 of 16, `expressions_equations` 10 of 14). Word problems + rationals also covered. Geometry and statistics still untouched.
- **Last updated:** 2026-05-17
- **Last action:** **#12 Equations / expressions (chunk 9)** — 5 more generators appended to `equation_generators.dart`: `solve_two_step_eq` (G7, `px ± q = r`, integer-x answers), `expand_linear_expression` (G7, "Expand 3(x + 4)" → "3x + 12"; MC over four expansion mistakes; prereq override drops the unimplemented `distributive_mult_over_add`), `add_subtract_linear_expressions` (G7, "Simplify 3x + 5 + (2x + 1)"; re-rolls degenerate cases; distractor pool excludes the "didn't apply sign to constant" variant when isPlus since it equals the correct answer), `equivalent_expressions_props` (G6, "Which is equivalent to 3(x + 2)?"; same override), `substitute_to_check` (G6, "Is x = 5 a solution to x + 3 = 8?" → Yes/No). 100 generators is a round number — and the expressions/equations category is now 10 of 14 covered. Tests: +5 groups @ 300; **279 total pass**, `flutter analyze` clean.
- **Last action (prior):** **#12 Equations (chunk 8) — entry** — `order_of_operations_no_exp`, `nested_grouping`, `evaluate_expression`, `solve_one_step_eq_addition`, `solve_one_step_eq_mult`. Commit `3602840`.
...
- **Next action:** Pick the next sub-slice. Easy options remaining ([curriculum.md §5.1](curriculum.md)):
  - **#16 Statistics** — `mean`, `median`, `mode`, `range_data`, `iqr`, `mad`. All algorithmic over a small list of numbers, no diagrams needed for the basic ones.
  - **Remaining equation rows** — `factor_linear_expression` (G7), `solve_two_step_eq_distributive` (G7), `solve_two_step_inequality` (G7, needs number_line), `solve_linear_eq_*` (G8).
  - **Remaining ratio rows** (need new widgets): `ratio_table`, `double_number_line`, `ratio_to_coordinate_pairs`, `multistep_ratio_word`, `scale_drawing`, `unit_rate_with_fractions`.
