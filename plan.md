...
- **Phase:** Phase 6 — Full Question Bank. **78 generators implemented**; two K–8 categories largely covered (`fractions` 26 + `decimals_percent` 22 of 26) plus solid word-problem coverage and Grade-7 rationals. Remaining decimals/percent rows: `convert_fraction_decimal_percent`, `decimals_fluent_4ops`, `decimal_on_number_line` (needs NumberLine extension), `repeating_decimal_*` (G8). Ratios (#8), geometry, prealgebra, and statistics still untouched.
- **Last updated:** 2026-05-17
- **Last action:** **#7 Decimals sub-slice (chunk 4) — finance percents** — 5 new generators appended to `percent_generators.dart`: `percent_change` (G7, increase/decrease variants, both ask for the percent magnitude), `simple_interest` (G7, I = P·R·T/100, principal multiple of 100), `commission` (G7, sale × rate / 100), `markup_markdown` (G7, two-context: "store buys for X, marks up" vs. "item costs X, markdown"; algorithmic despite curriculum.md tagging as dataset, per design principle 4 / `mult_compare_word` precedent), `sales_tax_tip` (G7, two-context: "meal cost X tip Y%" vs. "purchase X tax Y%"; algorithmic for same reason). All five answers are integer dollar/percent values — no decimals. Parameter discipline (friendly rates × step-divisible quantities) guarantees whole-number results. Tests: +5 groups @ 300 iterations; 257 total pass. `flutter analyze` clean.
...
- **Next action:** Pick the next sub-slice. Easy options remaining ([curriculum.md §5.1](curriculum.md)):
  - **#7 Decimals — chunk 5 (cleanup)** — `convert_fraction_decimal_percent` (F↔D↔% conversion table), `decimals_fluent_4ops` (composite practice mixing +,−,×,÷ on decimals). Both algorithmic, no new widgets.
