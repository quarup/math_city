...
- **Next action:** Pick the next sub-slice. What's still missing ([curriculum.md §5.1](curriculum.md)):
  ...
  - **Remaining G8 functions** — `solve_linear_eq_no_or_inf` (no/infinite-solution recognition), `compare_functions_representations` (compare slope/intercept across equation+points+table reps). These are pure-algorithmic — no widget required. The G8 linear-equation chain landed in Chunk 21: `slope_from_two_points`, `linear_function_construct`, `function_definition_check`, `solve_linear_eq_with_distrib_collect`, `solve_system_substitution`, `solve_system_elimination`.
