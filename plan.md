...
- **Phase:** Phase 6 — Full Question Bank. **80 generators implemented**; two K–8 categories largely covered (`fractions` 26 + `decimals_percent` 24 of 26) plus solid word-problem coverage and Grade-7 rationals. Remaining decimals/percent rows: `decimal_on_number_line` (needs NumberLine spec extension to support decimal markedPoints), `repeating_decimal_*` (G8). Ratios (#8), geometry, prealgebra, and statistics still untouched.
- **Last updated:** 2026-05-17
- **Last action:** **#7 Decimals sub-slice (chunk 5) — cleanup** — 2 new generators: `convert_fraction_decimal_percent` (G6) in `percent_generators.dart` — 6-variant cross-form converter (F↔D, F↔%, D↔% with all directions); per-variant `AnswerFormat` switching (`fraction` + `exactString` for "lowest terms" target, `decimal` for D target, `integer` for % target); misconception distractors are the *other* forms of the same value (a kid writing "25" when asked for "0.25" gets pre-emptively caught). `decimals_fluent_4ops` (G6) in `decimal_generators.dart` — composite that uniformly delegates to one of `addDecimals`, `subDecimals`, `multDecimals`, `divByDecimal` and retags the result with the composite concept ID. Tests: +2 groups; 259 total pass. `flutter analyze` clean.
- **Last action (prior):** **#7 Decimals sub-slice (chunk 4) — finance percents** — 5 new generators appended to `percent_generators.dart`: `percent_change`, `simple_interest`, `commission`, `markup_markdown`, `sales_tax_tip`. Commit `387d4ad`.
...
- **Next action:** Pick the next sub-slice. Easy options remaining ([curriculum.md §5.1](curriculum.md)):
  - **#8 Ratios / proportions** — `ratio_equivalent`, `unit_rate`, `solve_proportion`, `equivalent_ratios`, `ratio_table`, `proportional_relationship`, `constant_of_proportionality`, `proportional_equation`. Smaller surface, no diagrams needed (tape_diagram/double_number_line are optional polish).
